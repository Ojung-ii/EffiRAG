...
# Next-method-design family: 2Wiki entity-chunk grounding ablation.
...
if [ "${RETRIEVAL_ONLY}" = "true" ]; then
  ...
  OUTPUT_ROOT="${OUTPUT_ROOT:-outputs/retrieval_experiments/next_method_design/2wiki_grounding}"
else
  RUN_KIND="qa"
  RUN_QA="true"
  CFG_PREFIX="rag"
  OUTPUT_ROOT="${OUTPUT_ROOT:-outputs/rag_experiments/next_method_design/2wiki_grounding}"
fi
PROFILE_ROOT="${PROFILE_ROOT:-outputs/profiling/next_method_design/2wiki_grounding}"
...
printf '[nextdesign:2wiki_grounding] mode=%s limit=%s branch=%s commit=%s\n' "${RUN_KIND}" "${LIMIT}" "${GIT_BRANCH}" "${GIT_COMMIT}"

for variant in "${variants[@]}"; do
  CFG="configs/${CFG_PREFIX}_nextdesign_2wiki_grounding_${variant}.yaml"
  if [ ! -f "${CFG}" ]; then
  ...
  PROFILE_OUT="${PROFILE_ROOT}/nextdesign_2wiki_grounding_${variant}_${RUN_KIND}_q${LIMIT}_${RUN_STAMP}.jsonl"
...
REPORT_JSON="${PROFILE_ROOT}/nextdesign_2wiki_grounding_${RUN_KIND}_summary_q${LIMIT}_${RUN_STAMP}.json"
REPORT_MD="${PROFILE_ROOT}/nextdesign_2wiki_grounding_${RUN_KIND}_summary_q${LIMIT}_${RUN_STAMP}.md"
ANALYSIS_MD="${PROFILE_ROOT}/nextdesign_2wiki_grounding_${RUN_KIND}_interpretation_q${LIMIT}_${RUN_STAMP}.md"

SUMMARY_ARGS=(
  --schema semantic
  --title "Next Method Design: 2Wiki Grounding Ablation (${RUN_KIND})"
  --baseline-variant baseline
  --output-json "${REPORT_JSON}"
  --output-md "${REPORT_MD}"
  --analysis-md "${ANALYSIS_MD}"
  --experiment-family next_method_design
  --question-being-answered "Does entity-chunk grounding improve 2Wiki quality beyond seed/run tweaks?"
  --baseline-reference "2wiki_grounding_baseline"
  --frozen-config-reference "configs/rag_speed_profile.yaml + configs/rag_quality_profile.yaml"
  --dataset-scope "2wikimultihopqa"
  --changed-components seed_chunk_grounding
  --changed-components corridor_chunk_support
  --changed-components corridor_answer_alignment
...
cp -f "${REPORT_JSON}" "${PROFILE_ROOT}/nextdesign_2wiki_grounding_${RUN_KIND}_summary_latest.json"
cp -f "${REPORT_MD}" "${PROFILE_ROOT}/nextdesign_2wiki_grounding_${RUN_KIND}_summary_latest.md"
cp -f "${ANALYSIS_MD}" "${PROFILE_ROOT}/nextdesign_2wiki_grounding_${RUN_KIND}_interpretation_latest.md"

echo "[nextdesign:2wiki_grounding] done"
