...
def _reference_eval_src_candidates(repo_root: Path) -> List[Path]:
    return [
        repo_root / "third_party" / "HippoRAG2" / "src" / "hipporag",
    ]


def _resolve_reference_eval_src(repo_root: Path) -> Optional[Path]:
    for candidate in _reference_eval_src_candidates(repo_root):
        if candidate.exists():
            return candidate
    return None
...
def _load_reference_eval_metrics(repo_root: Path):
    try:
        reference_eval_src = _resolve_reference_eval_src(repo_root)
        if reference_eval_src is None:
            return None, None
...
def _extract_from_md_summary_paths(md_path: Path) -> List[Path]:
    ...
    paths = []
    for match in re.finditer(r"(/?[^\s|`]*rag_summary\.json)", text):
        p = Path(match.group(1))
        if p.is_absolute():
            if p.exists():
                paths.append(p.resolve())
            continue
        local = (Path(__file__).resolve().parents[1] / p).resolve()
        if local.exists():
            paths.append(local)
    return paths
...
def main() -> int:
    ...
    reference_eval_src = repo_root / "third_party" / "HippoRAG2" / "src"
    if reference_eval_src.exists():
        sys.path.insert(0, str(reference_eval_src))
    ...
    reference_em_metric, reference_f1_metric = _load_reference_eval_metrics(repo_root)
    reference_loaded = (reference_em_metric is not None) and (reference_f1_metric is not None)
    ...
    for summary_path in sorted(summary_paths):
        ...
        reference_em = None
        reference_f1 = None
        if reference_loaded and qa_scores["qa_count"] > 0:
            try:
                em_res, _ = reference_em_metric.calculate_metric_scores(qa_scores["gold_batch"], qa_scores["preds"])
                f1_res, _ = reference_f1_metric.calculate_metric_scores(qa_scores["gold_batch"], qa_scores["preds"])
                reference_em = _safe_float(em_res.get("ExactMatch", 0.0), 0.0)
                reference_f1 = _safe_float(f1_res.get("F1", 0.0), 0.0)
            except Exception:
                reference_em = None
                reference_f1 = None
        ...
        row = {
            ...
            "hipporag2_ref_EM": reference_em,
            "hipporag2_ref_F1": reference_f1,
            "delta_parity_vs_hipporag2_ref_EM": (None if reference_em is None else parity_em - reference_em),
            "delta_parity_vs_hipporag2_ref_F1": (None if reference_f1 is None else parity_f1 - reference_f1),
