...
if [[ -n "${RESUME_ROUND_ROOT:-}" ]]; then
  ...
  ROUND_ROOT="outputs/overnight_role_aware_refinement/${RUN_STAMP}"
fi

LOG_ROOT="logs/overnight_role_aware_refinement/$(basename "${ROUND_ROOT}")"
...
maybe_snapshot_git_state() {
  ...
  if [[ "${ENABLE_GIT_SNAPSHOT:-false}" != "true" ]]; then
    ...
    log_msg "[INFO]   git commit -m 'chore: snapshot before role-aware retrieval refinement'"
    log_msg "[INFO]   git checkout -b retrieval-objective-refine-v1"
    return 0
  fi

  git add -A
  if ! git diff --cached --quiet; then
    git commit -m "chore: snapshot before role-aware retrieval refinement"
  else
    log_msg "[INFO] No staged changes to commit before redesign branch."
  fi
  git checkout -b retrieval-objective-refine-v1 2>/dev/null || git checkout retrieval-objective-refine-v1
...
if mode == 'r2_plus_r3':
    cfg['retrieval_objective_mode'] = 'role_aware_chunk_scoring'
    cfg['bridge_candidate_induction_enabled'] = True
    cfg['role_aware_chunk_scoring_enabled'] = True
if mode == 'r4_minimal':
    cfg['retrieval_objective_mode'] = 'coverage_selection'
    cfg['coverage_selection_enabled'] = True
...
if tag == 'r3_anchor_heavy':
    cfg['role_aware_chunk_scoring_enabled'] = True
    cfg['retrieval_objective_mode'] = 'role_aware_chunk_scoring'
    cfg['role_chunk_weight_anchor_match'] = 0.45
    cfg['role_chunk_weight_bridge_support'] = 0.25
    cfg['role_chunk_weight_answer_likelihood'] = 0.20
    cfg['role_chunk_weight_path_consistency'] = 0.10
elif tag == 'r3_bridge_heavy':
    cfg['role_aware_chunk_scoring_enabled'] = True
    cfg['retrieval_objective_mode'] = 'role_aware_chunk_scoring'
    cfg['role_chunk_weight_anchor_match'] = 0.25
    cfg['role_chunk_weight_bridge_support'] = 0.45
    cfg['role_chunk_weight_answer_likelihood'] = 0.20
    cfg['role_chunk_weight_path_consistency'] = 0.10
elif tag == 'r3_answer_heavy':
    cfg['role_aware_chunk_scoring_enabled'] = True
    cfg['retrieval_objective_mode'] = 'role_aware_chunk_scoring'
    cfg['role_chunk_weight_anchor_match'] = 0.25
    cfg['role_chunk_weight_bridge_support'] = 0.25
    cfg['role_chunk_weight_answer_likelihood'] = 0.40
    cfg['role_chunk_weight_path_consistency'] = 0.10
elif tag == 'r3_path_heavy':
    cfg['role_aware_chunk_scoring_enabled'] = True
    cfg['retrieval_objective_mode'] = 'role_aware_chunk_scoring'
    cfg['role_chunk_weight_anchor_match'] = 0.25
    cfg['role_chunk_weight_bridge_support'] = 0.25
    cfg['role_chunk_weight_answer_likelihood'] = 0.20
    cfg['role_chunk_weight_path_consistency'] = 0.30
elif tag == 'r4_minimal':
    cfg['coverage_selection_enabled'] = True
    cfg['retrieval_objective_mode'] = 'coverage_selection'
    cfg['coverage_selection_role_weight'] = 0.20
    cfg['coverage_selection_redundancy_weight'] = 0.30
...
lines = [
    '# Overnight Role-Aware Objective Refinement Metrics (RAG full)',
    '',
...
payload = {
    ...
    "fixed_by": "manual_freeze_for_role_aware_refinement",
...
log_msg "[Stage V0] baseline_mid_reconfirm"
run_variant_for_dataset "v0" "baseline_mid_reconfirm" "hotpotqa" "hotpot_f2" "${UNIVERSAL_REGIME}" "baseline" "hotpot_fixed" 4 1 "${LIMIT_RAG}"
run_variant_for_dataset "v0" "baseline_mid_reconfirm" "2wikimultihopqa" "${WIKI_CORE_PROFILE}" "${UNIVERSAL_REGIME}" "baseline" "wiki_fixed" 3 1 "${LIMIT_RAG}"

log_msg "[Stage V1] bridge vs role decomposition"
for mode in \
  "bridge_candidate_induction:r2_bridge_only" \
  "role_aware_chunk_scoring:r3_role_only" \
  "r2_plus_r3:r2_plus_r3"
do
  retrieval_mode="${mode%%:*}"
  variant="${mode##*:}"
  log_msg "[V1] ${variant} hotpot"
  run_variant_for_dataset "v1" "${variant}" "hotpotqa" "hotpot_f2" "${UNIVERSAL_REGIME}" "${retrieval_mode}" "hotpot_fixed" 4 1 "${LIMIT_RAG}"
  log_msg "[V1] ${variant} 2wiki"
  run_variant_for_dataset "v1" "${variant}" "2wikimultihopqa" "${WIKI_CORE_PROFILE}" "${UNIVERSAL_REGIME}" "${retrieval_mode}" "wiki_fixed" 3 1 "${LIMIT_RAG}"
done

log_msg "[Stage V2] role-aware internal weight sweep"
for variant in \
  "r3_anchor_heavy" \
  "r3_bridge_heavy" \
  "r3_answer_heavy" \
  "r3_path_heavy"
do
  log_msg "[V2] ${variant} hotpot"
  run_variant_for_dataset "v2" "${variant}" "hotpotqa" "hotpot_f2" "${UNIVERSAL_REGIME}" "role_aware_chunk_scoring" "hotpot_fixed" 4 1 "${LIMIT_RAG}"
  log_msg "[V2] ${variant} 2wiki"
  run_variant_for_dataset "v2" "${variant}" "2wikimultihopqa" "${WIKI_CORE_PROFILE}" "${UNIVERSAL_REGIME}" "role_aware_chunk_scoring" "wiki_fixed" 3 1 "${LIMIT_RAG}"
done

log_msg "[Stage V3] coverage minimal check"
run_variant_for_dataset "v3" "r4_minimal" "hotpotqa" "hotpot_f2" "${UNIVERSAL_REGIME}" "r4_minimal" "hotpot_fixed" 4 1 "${LIMIT_RAG}"
run_variant_for_dataset "v3" "r4_minimal" "2wikimultihopqa" "${WIKI_CORE_PROFILE}" "${UNIVERSAL_REGIME}" "r4_minimal" "wiki_fixed" 3 1 "${LIMIT_RAG}"
