...
OUT_ROOT="${OUT_ROOT:-outputs/phase6u_abr_objective_redesign_n100}"
...
ROLE_BALANCED_PROFILE="${ROLE_BALANCED_PROFILE:-unified_acr_rcedr_v12_role_balanced}"
...
echo "=== PHASE6U-3 ABR objective redesign (GPU1 / 2 proc) ==="
echo "OUT_ROOT=${OUT_ROOT}"
echo "SAMPLE_SIZE=${SAMPLE_SIZE}"
echo "PROCESS_COUNT=${PROCESS_COUNT}"
echo "GPU=${GPU}"
echo "BASELINE_PROFILE=${BASELINE_PROFILE}"
echo "ROLE_BALANCED_PROFILE=${ROLE_BALANCED_PROFILE}"
...
SCHEDULE_JSONL="${OUT_ROOT}/_phase6u_abr_objective_redesign_scheduled_runs.jsonl"
: > "${SCHEDULE_JSONL}"
...
register_run() {
  ...
  local group="$4"
  local role="$5"
  printf '{"phase":"phase6u_abr_objective_redesign","run_name":"%s","dataset":"%s","profile":"%s","gpu":"%s","process_group":"%s","role":"%s","kind":"unified","scheduled":true}\n' \
    "${run_name}" "${dataset}" "${profile}" "${GPU}" "${group}" "${role}" >> "${SCHEDULE_JSONL}"
}

TASKS_A=(
  "${BASELINE_PROFILE}|hotpotqa|v12_hotpotqa|A|baseline"
  "${ROLE_BALANCED_PROFILE}|hotpotqa|v12_role_balanced_hotpotqa|A|variant_role_balanced"
...
TASKS_B=(
  ...
  "${ROLE_BALANCED_PROFILE}|2wikimultihopqa|v12_role_balanced_2wikimultihopqa|B|variant_role_balanced"
...
echo "=== summarize PHASE6U ABR objective redesign ==="
PYTHONPATH=. "${PYTHON}" scripts/summarize_phase6u_abr_objective_redesign.py \
  --out-root "${OUT_ROOT}" \
  --datasets hotpotqa,2wikimultihopqa \
  --baseline-profile "${BASELINE_PROFILE}" \
  --variant-profiles "${ROLE_BALANCED_PROFILE},${REDUNDANCY_PROFILE}" \
  --top-k 20 \
  2>&1 | tee "${OUT_ROOT}/logs/summarize_phase6u_abr_objective_redesign.log"
...
echo "=== done ==="
echo "Summary: ${OUT_ROOT}/PHASE6U_ABR_OBJECTIVE_REDESIGN_SUMMARY.md"
echo "Summary JSON: ${OUT_ROOT}/phase6u_abr_objective_redesign_summary.json"
