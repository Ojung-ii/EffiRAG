...
GPU_A="${GPU_A:-0}"  # hotpotqa
GPU_B="${GPU_B:-0}"  # 2wikimultihopqa
GPU_C="${GPU_C:-1}"  # musique
GPU_D="${GPU_D:-1}"  # popqa
...
if [[ "${PROCESS_COUNT}" != "4" ]]; then
  echo "[ERROR] PROCESS_COUNT must be 4, got: ${PROCESS_COUNT}" >&2
  exit 1
fi
...
# Backward-compatible mirror for tooling expecting shared_sample_ids
...
group_c() {
  run_one "${GPU_C}" "phase6x_musique_v12" "${OUT_ROOT}/configs/phase6x_musique_v12.yaml"
  run_one "${GPU_C}" "phase6x_musique_v12_early_collapse_recovery" "${OUT_ROOT}/configs/phase6x_musique_v12_early_collapse_recovery.yaml"
}

group_d() {
  run_one "${GPU_D}" "phase6x_popqa_v12" "${OUT_ROOT}/configs/phase6x_popqa_v12.yaml"
  run_one "${GPU_D}" "phase6x_popqa_v12_early_collapse_recovery" "${OUT_ROOT}/configs/phase6x_popqa_v12_early_collapse_recovery.yaml"
}
