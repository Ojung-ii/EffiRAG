...
LIMIT="${LIMIT:-100}"
OUT_ROOT="${OUT_ROOT:-outputs/phase7_evidence_flow/hotpot_2wiki_n100}"

cd "${REPO_ROOT}"
mkdir -p "${OUT_ROOT}" "${OUT_ROOT}/logs"

echo "[phase7-n100] audit"
...
run_one() {
  ...
  mkdir -p "${out_dir}"
  echo "[phase7-n100] dataset=${dataset} limit=${LIMIT} out=${out_dir}"
...
echo "[phase7-n100] done: ${OUT_ROOT}"
