...
VARIANTS_CSV="champion_reconfirm,light_separator_render"
APPENDIX_VARIANTS_CSV="hotpotqa:inline_source_role_hint_hotpot_only"
N_SAMPLES="1000"
QA_ROOT="data/qa"
CORPUS_ROOT="/home/ojungii/HippoRAG2/dataset"
EFFIRAG_CHAMPION_OUTPUT_ROOT="outputs/retrieval_metric_hotpot_2wiki_round_rerun_strict"
OUTPUT_ROOT="outputs/light_separator_lockin_round"
...
usage() {
  cat <<'USAGE'
Usage: bash scripts/run_light_separator_lockin_round.sh [options]

Primary:
  --datasets CSV
  --variants CSV
  --appendix-variants CSV
  --n-samples N
  --qa-root PATH
  --corpus-root PATH
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --appendix-variants) APPENDIX_VARIANTS_CSV="$2"; shift 2 ;;
...
"${PYTHON_BIN}" scripts/light_separator_lockin_round.py \
  --datasets "${DATASETS_CSV}" \
  --variants "${VARIANTS_CSV}" \
  --appendix-variants "${APPENDIX_VARIANTS_CSV}" \
  --n-samples "${N_SAMPLES}" \
  --qa-root "${QA_ROOT}" \
