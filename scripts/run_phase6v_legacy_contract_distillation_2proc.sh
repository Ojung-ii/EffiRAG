...
OUT_ROOT="${OUT_ROOT:-outputs/phase6v_legacy_contract_distillation_n100}"
...
SEM_PROFILE="${SEM_PROFILE:-unified_acr_rcedr_v12_semantic_sufficiency}"
CONTRACT_PROFILE="${CONTRACT_PROFILE:-unified_acr_rcedr_v12_semantic_contract}"
...
echo "=== PHASE6V legacy contract distillation n100 (GPU1 / 2 proc) ==="
echo "OUT_ROOT=${OUT_ROOT}"
echo "SAMPLE_SIZE=${SAMPLE_SIZE}"
echo "PROCESS_COUNT=${PROCESS_COUNT}"
echo "GPU=${GPU}"
echo "BASELINE_PROFILE=${BASELINE_PROFILE}"
echo "SEM_PROFILE=${SEM_PROFILE}"
echo "CONTRACT_PROFILE=${CONTRACT_PROFILE}"

if command -v nvidia-smi >/dev/null 2>&1; then
  nvidia-smi > "${OUT_ROOT}/logs/nvidia_smi_before.txt" || true
fi

SCHEDULE_JSONL="${OUT_ROOT}/_phase6v_legacy_contract_scheduled_runs.jsonl"
: > "${SCHEDULE_JSONL}"
...
register_run() {
  ...
  local group="$4"
  local role="$5"
  printf '{"phase":"phase6v_legacy_contract_distillation","run_name":"%s","dataset":"%s","profile":"%s","gpu":"%s","process_group":"%s","role":"%s","kind":"unified","scheduled":true}\n' \
...
TASKS_A=(
  ...
  "${CONTRACT_PROFILE}|hotpotqa|v12_semantic_contract_hotpotqa|A|semantic_contract"
)
TASKS_B=(
  "${BASELINE_PROFILE}|2wikimultihopqa|v12_2wikimultihopqa|B|baseline"
  "${SEM_PROFILE}|2wikimultihopqa|v12_semantic_sufficiency_2wikimultihopqa|B|semantic_sufficiency"
  "${CONTRACT_PROFILE}|2wikimultihopqa|v12_semantic_contract_2wikimultihopqa|B|semantic_contract"
)
...
echo "=== summarize PHASE6V legacy contract distillation ==="
PYTHONPATH=. "${PYTHON}" scripts/summarize_phase6v_legacy_contract_distillation.py \
  --out-root "${OUT_ROOT}"
