...
OUT_ROOT="${OUT_ROOT:-outputs/phase6w_pamae_corridor_consistency_n100}"
...
RUN_A_NAME="${RUN_A_NAME:-phase6w_pamae_hotpotqa_v12}"
RUN_B_NAME="${RUN_B_NAME:-phase6w_pamae_2wikimultihopqa_v12}"
...
echo "=== PHASE6W-P PAMAE corridor consistency audit (GPU1 / 2 proc) ==="
echo "OUT_ROOT=${OUT_ROOT}"
echo "SAMPLE_SIZE=${SAMPLE_SIZE}"
echo "PROCESS_COUNT=${PROCESS_COUNT}"
echo "GPU=${GPU}"
echo "PROFILE=${PROFILE}"
echo "PHASE6W_PAMAE_AUDIT=${PHASE6W_PAMAE_AUDIT:-1}"

if command -v nvidia-smi >/dev/null 2>&1; then
  nvidia-smi > "${OUT_ROOT}/logs/nvidia_smi_before.txt" || true
fi

SCHEDULE_JSONL="${OUT_ROOT}/_phase6w_pamae_corridor_consistency_scheduled_runs.jsonl"
: > "${SCHEDULE_JSONL}"
...
register_run() {
  ...
  local run_name="$2"
  local group="$3"
  printf '{"phase":"phase6w_pamae_corridor_consistency","run_name":"%s","dataset":"%s","profile":"%s","gpu":"%s","process_group":"%s","role":"corridor_consistency_audit","kind":"unified","scheduled":true}\n' \
...
phase6w = out_root / "phase6w_run_manifest.json"
phase6s = out_root / "phase6s_run_manifest.json"
phase6w.write_text(json.dumps(manifest, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
...
run_unified_qa_one_dataset() {
  ...
  PHASE6W_PAMAE_AUDIT="${PHASE6W_PAMAE_AUDIT:-1}" \
  PYTHONHASHSEED=0 \
...
echo "=== summarize PHASE6W-P corridor consistency ==="
PYTHONPATH=. "${PYTHON}" scripts/summarize_phase6w_pamae_corridor_consistency.py \
  --out-root "${OUT_ROOT}" \
  --datasets hotpotqa,2wikimultihopqa \
  --profile "${PROFILE}" \
  --top-k 20 \
  2>&1 | tee "${OUT_ROOT}/logs/summarize_phase6w_pamae_corridor_consistency.log"
...
echo "=== done ==="
echo "Summary: ${OUT_ROOT}/PHASE6W_PAMAE_CORRIDOR_CONSISTENCY_SUMMARY.md"
echo "Summary JSON: ${OUT_ROOT}/phase6w_pamae_corridor_consistency_summary.json"
echo "By-query JSONL: ${OUT_ROOT}/corridor_consistency_by_query.jsonl"
echo "By-dataset CSV: ${OUT_ROOT}/corridor_consistency_by_dataset.csv"
echo "Query-type CSV: ${OUT_ROOT}/query_type_corridor_breakdown.csv"
