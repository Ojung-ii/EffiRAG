...
if [[ -n "${RESUME_ROUND_ROOT:-}" ]]; then
  ...
  ROUND_ROOT="outputs/overnight_pamae_seed_run_corridor/${RUN_STAMP}"
fi

LOG_ROOT="logs/overnight_pamae_seed_run_corridor/$(basename "${ROUND_ROOT}")"
...
maybe_snapshot_git_state() {
  ...
  if [[ "${ENABLE_GIT_SNAPSHOT:-false}" != "true" ]]; then
    ...
    log_msg "[INFO]   git commit -m 'chore: snapshot before PAMAE seed-run-corridor refinement'"
    log_msg "[INFO]   git checkout -b pamae-seed-run-corridor-v1"
    return 0
  fi

  git add -A
  if ! git diff --cached --quiet; then
    git commit -m "chore: snapshot before PAMAE seed-run-corridor refinement"
  else
    log_msg "[INFO] No staged changes to commit before redesign branch."
  fi
  git checkout -b pamae-seed-run-corridor-v1 2>/dev/null || git checkout pamae-seed-run-corridor-v1
...
lines = [
    '# Overnight PAMAE Seed-Run-Corridor Metrics (RAG full)',
    '',
    '| stage | variant | dataset | R@5 | EM | F1 | bridge_purity | answer_side_density | support_set_compactness | bridge_noise_ratio | useful_bridge_rate | bridge_to_answer_path_hit | conversion_after_bridge | answer_conversion | f1_per_r5 | err_bridge_missing | err_answer_missing | err_bridge_noisy | err_gen_fail | oracle_gap | gap_to_hippo_r5 | gap_to_hippo_f1 |',
    '|---|---|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|',
]
for r in rows:
    lines.append(
        f"| {r['stage']} | {r['variant']} | {r['dataset']} | {r['Recall@5']:.4f} | {r['EM']:.4f} | {r['F1']:.4f} | {r['bridge_purity']:.4f} | {r['answer_side_density']:.4f} | {r['support_set_compactness']:.4f} | {r['bridge_noise_ratio']:.4f} | {r['useful_bridge_rate']:.4f} | {r['bridge_to_answer_path_hit']:.4f} | {r['conversion_after_bridge']:.4f} | {r['answer_conversion']:.4f} | {r['f1_per_r5']:.4f} | {r['error_bucket_bridge_missing']:.4f} | {r['error_bucket_answer_side_missing']:.4f} | {r['error_bucket_bridge_present_noisy']:.4f} | {r['error_bucket_answer_present_generation_fail']:.4f} | {r['oracle_gap']:+.4f} | {r['gap_to_hippo_r5']:+.4f} | {r['gap_to_hippo_f1']:+.4f} |"
    )
Path(out_md).write_text('\n'.join(lines) + '\n', encoding='utf-8')
PY
}
...
payload = {
    ...
    "fixed_by": "manual_freeze_for_pamae_seed_run_corridor_round",
...
run_variant_for_dataset "p0" "baseline_mid_reconfirm" "hotpotqa" "hotpot_f2" "${UNIVERSAL_REGIME}" "baseline" "off" 0 0 "${LIMIT_RAG}"
run_variant_for_dataset "p0" "baseline_mid_reconfirm" "2wikimultihopqa" "${WIKI_CORE_PROFILE}" "${UNIVERSAL_REGIME}" "baseline" "off" 0 0 "${LIMIT_RAG}"

log_msg "[Stage P1] seed_run_connector_core"
run_variant_for_dataset "p1" "seed_run_connector_core" "hotpotqa" "hotpot_f2" "${UNIVERSAL_REGIME}" "seed_run_connector_core" "off" 0 0 "${LIMIT_RAG}"
run_variant_for_dataset "p1" "seed_run_connector_core" "2wikimultihopqa" "${WIKI_CORE_PROFILE}" "${UNIVERSAL_REGIME}" "seed_run_connector_core" "off" 0 0 "${LIMIT_RAG}"

log_msg "[Stage P2] seed_run_connector_core + corridor_compact"
run_variant_for_dataset "p2" "seed_run_connector_core_corridor_compact" "hotpotqa" "hotpot_f2" "${UNIVERSAL_REGIME}" "seed_run_connector_core_corridor_compact" "off" 0 0 "${LIMIT_RAG}"
run_variant_for_dataset "p2" "seed_run_connector_core_corridor_compact" "2wikimultihopqa" "${WIKI_CORE_PROFILE}" "${UNIVERSAL_REGIME}" "seed_run_connector_core_corridor_compact" "off" 0 0 "${LIMIT_RAG}"

log_msg "[Stage P3] seed_run_connector_core + corridor_answer_preserve"
run_variant_for_dataset "p3" "seed_run_connector_core_corridor_answer_preserve" "hotpotqa" "hotpot_f2" "${UNIVERSAL_REGIME}" "seed_run_connector_core_corridor_answer_preserve" "off" 0 0 "${LIMIT_RAG}"
run_variant_for_dataset "p3" "seed_run_connector_core_corridor_answer_preserve" "2wikimultihopqa" "${WIKI_CORE_PROFILE}" "${UNIVERSAL_REGIME}" "seed_run_connector_core_corridor_answer_preserve" "off" 0 0 "${LIMIT_RAG}"

log_msg "[Stage P4] seed_run_connector_core + corridor_bridge_purity"
run_variant_for_dataset "p4" "seed_run_connector_core_corridor_bridge_purity" "hotpotqa" "hotpot_f2" "${UNIVERSAL_REGIME}" "seed_run_connector_core_corridor_bridge_purity" "off" 0 0 "${LIMIT_RAG}"
run_variant_for_dataset "p4" "seed_run_connector_core_corridor_bridge_purity" "2wikimultihopqa" "${WIKI_CORE_PROFILE}" "${UNIVERSAL_REGIME}" "seed_run_connector_core_corridor_bridge_purity" "off" 0 0 "${LIMIT_RAG}"

log_msg "[Stage P5] seed_run_connector_core + corridor_compact + answer_preserve"
run_variant_for_dataset "p5" "seed_run_connector_core_corridor_compact_answer_preserve" "hotpotqa" "hotpot_f2" "${UNIVERSAL_REGIME}" "seed_run_connector_core_corridor_compact_answer_preserve" "off" 0 0 "${LIMIT_RAG}"
run_variant_for_dataset "p5" "seed_run_connector_core_corridor_compact_answer_preserve" "2wikimultihopqa" "${WIKI_CORE_PROFILE}" "${UNIVERSAL_REGIME}" "seed_run_connector_core_corridor_compact_answer_preserve" "off" 0 0 "${LIMIT_RAG}"
