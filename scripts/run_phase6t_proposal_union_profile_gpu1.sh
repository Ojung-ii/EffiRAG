...
OUT_ROOT="${OUT_ROOT:-outputs/phase6t_proposal_union_profile_hotpotqa_n20}"
...
if [[ "${PROCESS_COUNT}" != "1" ]]; then
  echo "[ERROR] PROCESS_COUNT must be 1 for clean proposal-union profiling." >&2
  exit 1
fi
...
echo "=== Phase-6T proposal-union deep profile (GPU1-only) ==="
...
PYTHONPATH=. "${PYTHON}" scripts/audit_phase6t_latency_triage.py \
  ...
  --out-root "${OUT_ROOT}" \
  2>&1 | tee "${OUT_ROOT}/logs/audit_phase6t_proposal_union.log"

SCHEDULE_JSONL="${OUT_ROOT}/_phase6t_proposal_union_scheduled_runs.jsonl"
: > "${SCHEDULE_JSONL}"
printf '{"run_name":"%s","dataset":"%s","profile":"%s","gpu":"%s","process_group":"A","role":"proposal_union_profile","kind":"unified","scheduled":true}\n' \
...
echo "=== summarize Phase-6T proposal-union profile ==="
PYTHONPATH=. "${PYTHON}" scripts/summarize_phase6t_proposal_union_profile.py \
  --out-root "${OUT_ROOT}" \
  --dataset "${DATASET}" \
  --profile "${PROFILE}" \
  2>&1 | tee "${OUT_ROOT}/logs/summarize_phase6t_proposal_union.log"
...
echo "=== done ==="
echo "Summary: ${OUT_ROOT}/PHASE6T_PROPOSAL_UNION_PROFILE.md"
echo "JSON: ${OUT_ROOT}/proposal_union_profile_summary.json"
