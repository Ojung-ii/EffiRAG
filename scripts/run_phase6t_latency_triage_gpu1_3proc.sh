...
OUT_ROOT="${OUT_ROOT:-outputs/phase6t_latency_triage_n50_gpu1_3proc}"
...
echo "=== Phase-6T SOTA-contract latency triage (GPU1-only) ==="
echo "OUT_ROOT=${OUT_ROOT}"
echo "SAMPLE_SIZE=${SAMPLE_SIZE}"
echo "PROCESS_COUNT=${PROCESS_COUNT}"
echo "GPU=${GPU}"

echo "=== Phase-6T latency triage audit gate ==="
PYTHONPATH=. "${PYTHON}" scripts/audit_phase6t_latency_triage.py \
  ...
  --out-root "${OUT_ROOT}" \
  2>&1 | tee "${OUT_ROOT}/logs/audit_phase6t_latency_triage.log"

SCHEDULE_JSONL="${OUT_ROOT}/_phase6t_latency_scheduled_runs.jsonl"
: > "${SCHEDULE_JSONL}"
...
register_run () {
  ...
  local group="$4"
  local role="$5"
  printf '{"run_name":"%s","dataset":"%s","profile":"%s","gpu":"%s","process_group":"%s","role":"%s","kind":"unified","scheduled":true}\n' \
...
if [[ "${PROCESS_COUNT}" == "3" ]]; then
  TASKS_A=(
    "unified_acr_rcedr_v12|hotpotqa|v12_current_hotpotqa|A|baseline_current"
    "unified_acr_rcedr_v12_sota_contract_fast|2wikimultihopqa|v12_sota_contract_fast_2wiki|A|latency_triage"
    "unified_acr_rcedr_v12_sota_contract_search3x3|hotpotqa|v12_sota_contract_search3x3_hotpotqa|A|latency_triage"
  )
  TASKS_B=(
    "unified_acr_rcedr_v12|2wikimultihopqa|v12_current_2wiki|B|baseline_current"
    "unified_acr_rcedr_v12_sota_contract_unified|hotpotqa|v12_sota_contract_unified_hotpotqa|B|contract_alignment"
    "unified_acr_rcedr_v12_sota_contract_rerank20|2wikimultihopqa|v12_sota_contract_rerank20_2wiki|B|latency_triage"
  )
  TASKS_C=(
    "unified_acr_rcedr_v12_sota_contract_unified|2wikimultihopqa|v12_sota_contract_unified_2wiki|C|contract_alignment"
    "unified_acr_rcedr_v12_sota_contract_rerank20|hotpotqa|v12_sota_contract_rerank20_hotpotqa|C|latency_triage"
    "unified_acr_rcedr_v12_sota_contract_search3x3|2wikimultihopqa|v12_sota_contract_search3x3_2wiki|C|latency_triage"
    "unified_acr_rcedr_v12_sota_contract_fast|hotpotqa|v12_sota_contract_fast_hotpotqa|C|latency_triage"
  )
else
  TASKS_A=(
    "unified_acr_rcedr_v12|hotpotqa|v12_current_hotpotqa|A|baseline_current"
    "unified_acr_rcedr_v12_sota_contract_unified|2wikimultihopqa|v12_sota_contract_unified_2wiki|A|contract_alignment"
    "unified_acr_rcedr_v12_sota_contract_rerank20|hotpotqa|v12_sota_contract_rerank20_hotpotqa|A|latency_triage"
    "unified_acr_rcedr_v12_sota_contract_search3x3|2wikimultihopqa|v12_sota_contract_search3x3_2wiki|A|latency_triage"
    "unified_acr_rcedr_v12_sota_contract_fast|hotpotqa|v12_sota_contract_fast_hotpotqa|A|latency_triage"
  )
  TASKS_B=(
    "unified_acr_rcedr_v12|2wikimultihopqa|v12_current_2wiki|B|baseline_current"
    "unified_acr_rcedr_v12_sota_contract_unified|hotpotqa|v12_sota_contract_unified_hotpotqa|B|contract_alignment"
    "unified_acr_rcedr_v12_sota_contract_rerank20|2wikimultihopqa|v12_sota_contract_rerank20_2wiki|B|latency_triage"
    "unified_acr_rcedr_v12_sota_contract_search3x3|hotpotqa|v12_sota_contract_search3x3_hotpotqa|B|latency_triage"
    "unified_acr_rcedr_v12_sota_contract_fast|2wikimultihopqa|v12_sota_contract_fast_2wiki|B|latency_triage"
  )
fi
...
for spec in "${ALL_TASKS[@]}"; do
  [[ -z "${spec}" ]] && continue
  IFS='|' read -r profile dataset run_name group role <<< "${spec}"
  register_run "${profile}" "${dataset}" "${run_name}" "${group}" "${role}"
done
...
echo "=== summarize Phase-6T latency triage ==="
PYTHONPATH=. "${PYTHON}" scripts/summarize_phase6t_latency_triage.py \
  --out-root "${OUT_ROOT}" \
  2>&1 | tee "${OUT_ROOT}/logs/summarize_phase6t_latency_triage.log"
...
echo "=== done ==="
echo "Audit: ${OUT_ROOT}/PHASE6T_LATENCY_TRIAGE_AUDIT.md"
echo "Summary: ${OUT_ROOT}/PHASE6T_LATENCY_TRIAGE_SUMMARY.md"
echo "Summary JSON: ${OUT_ROOT}/phase6t_latency_triage_summary.json"
