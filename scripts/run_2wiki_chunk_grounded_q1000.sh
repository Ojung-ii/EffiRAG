...
# 2Wiki q1000 revalidation for chunk-grounded corridor candidates.
...
LIMIT="${LIMIT:-1000}"
RUN_OPTIONAL_CORRIDOR="${RUN_OPTIONAL_CORRIDOR:-false}"
...
OUTPUT_ROOT="${OUTPUT_ROOT:-outputs/rag_experiments/next_method_design/chunk_grounded_context_q1000/2wikimultihopqa}"
...
if [ "${RUN_OPTIONAL_CORRIDOR}" = "true" ]; then
  variants+=(corridor_lift_grounded)
fi
...
printf '[2wiki_chunk_grounded_q1000] limit=%s optional_corridor=%s branch=%s commit=%s\n' "${LIMIT}" "${RUN_OPTIONAL_CORRIDOR}" "${GIT_BRANCH}" "${GIT_COMMIT}"

for variant in "${variants[@]}"; do
  CFG="configs/rag_exp_2wiki_chunk_grounded_q1000_${variant}.yaml"
  if [ ! -f "${CFG}" ]; then
...
REPORT_JSON="${PROFILE_ROOT}/chunk_grounded_context_2wikimultihopqa_q1000_summary_q${LIMIT}_${RUN_STAMP}.json"
REPORT_MD="${PROFILE_ROOT}/chunk_grounded_context_2wikimultihopqa_q1000_summary_q${LIMIT}_${RUN_STAMP}.md"
INTERP_MD="${PROFILE_ROOT}/chunk_grounded_context_2wikimultihopqa_q1000_interpretation_q${LIMIT}_${RUN_STAMP}.md"

SUMMARY_ARGS=(
  --title "Chunk-Grounded Context q1000 Revalidation (2wikimultihopqa, qa)"
  --baseline-variant baseline
  --output-json "${REPORT_JSON}"
  --output-md "${REPORT_MD}"
  --interpretation-md "${INTERP_MD}"
  --experiment-family next_method_design
  --question-being-answered "Are shallow package/backfill variants better than strong grounding on 2Wiki q1000?"
...
cp -f "${REPORT_JSON}" "${PROFILE_ROOT}/chunk_grounded_context_2wikimultihopqa_q1000_summary_latest.json"
cp -f "${REPORT_MD}" "${PROFILE_ROOT}/chunk_grounded_context_2wikimultihopqa_q1000_summary_latest.md"
cp -f "${INTERP_MD}" "${PROFILE_ROOT}/chunk_grounded_context_2wikimultihopqa_q1000_interpretation_latest.md"

echo "[2wiki_chunk_grounded_q1000] done"
