...
def _parse_appendix_variants(raw: str) -> Dict[str, List[str]]:
    # Format: dataset:variant,dataset2:variant2
...
def _variant_plan(variant: str) -> Dict[str, Any]:
    ...
    if vv == "inline_source_role_hint_hotpot_only":
        return {
            "render_mode": "corridor_aware_flat",
            "prompt_variant": "default",
            "order_flags": ["inline_source_role_hint"],
            "render_variant": "inline_source_role_hint_hotpot_only",
            "prompt_variant_label": "default",
            "variant_family": "appendix",
        }
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--variants", default="champion_reconfirm,light_separator_render")
    p.add_argument("--appendix-variants", default="hotpotqa:inline_source_role_hint_hotpot_only")
...
def main() -> None:
    ...
    appendix_by_dataset = _parse_appendix_variants(args.appendix_variants)
    if not datasets:
    ...
    champion_root = _discover_round_root(Path(args.effirag_champion_output_root).resolve())
    if champion_root is None:
        raise SystemExit(f"champion run_records.tsv not found under {args.effirag_champion_output_root}")
    ...
    champion_by_dataset: Dict[str, Optional[Dict[str, str]]] = {}
    for ds in datasets:
        champion_by_dataset[ds] = _find_champion_run(champion_root, ds)

    champion_manifest = {
        "generated_at_utc": datetime.now(timezone.utc).isoformat(),
        "champion_root": str(champion_root),
        "datasets": datasets,
        "champions": {ds: champion_by_dataset.get(ds) for ds in datasets},
    }
    write_json(str(champion_manifest_path), champion_manifest)
    ...
    for ds in datasets:
        row = champion_by_dataset.get(ds)
        if row is None:
            retrieval_fixed_manifest["datasets"].append(
                {
                    ...
                    "reason": "champion_config_missing_in_provided_root",
                }
            )
            continue
            ...
            retrieval_fixed_manifest["datasets"].append(
                {
                    ...
                    "champion_summary_path": str(summary_path),
                    "champion_query_path": str(query_path),
                    "champion_query_count": int(_count_jsonl_lines(query_path)),
                    "champion_config_path": str(Path(_strip(row.get("config_path"))).resolve()),
                }
            )
        else:
            retrieval_fixed_manifest["datasets"].append(
                {
                    ...
                    "champion_available": True,
                    "retrieval_fixed_available": False,
                    "reason": "champion_query_artifact_missing",
                    "champion_summary_path": str(summary_path),
                    "champion_config_path": str(Path(_strip(row.get("config_path"))).resolve()),
                }
            )
    ...
    env_snapshot = {
        ...
        "datasets": datasets,
        "variants": variants,
        "appendix_variants": appendix_by_dataset,
    ...
    for ds in datasets:
        ...
        champion_row = champion_by_dataset.get(ds)
        champion_available = bool(champion_row is not None)
        champion_source = "provided_root" if champion_available else "canonical_fallback"
        ...
        if champion_available:
            ...
            base_cfg = dict(champion_cfg)
            precomputed_retrieval_path = champion_summary_path.with_name("rag_query_results.jsonl")
            retrieval_fixed_expected = bool(precomputed_retrieval_path.exists())
        else:
            champion_cfg = None
            champion_summary_path = None
        ...
        run_variants = []
        for vv in variants:
            if vv not in run_variants:
                run_variants.append(vv)
        for vv in appendix_by_dataset.get(ds, []):
            if vv not in run_variants:
                run_variants.append(vv)

        for vv in run_variants:
            plan = _variant_plan(vv)
            family = str(plan.get("variant_family", "main"))
            stage = "s0" if vv == "champion_reconfirm" else ("s2" if family == "appendix" else "s1")
            profile = "champion_reconfirm" if vv == "champion_reconfirm" else ("appendix" if family == "appendix" else "light_separator_lockin")
                ...
                run_records.append(
                    {
                        ...
                        "champion_available": str(champion_available).lower(),
            ...
            with log_path.open("ab") as lf:
                header = (
                    f"[START] {start_ts}\n"
                    f"[COMMAND] {' '.join(cmd)}\n"
                    f"[CHAMPION_AVAILABLE] {str(champion_available).lower()}\n"
    ...
    agg_cmd = [
        sys.executable,
        str(REPO_ROOT / "scripts" / "aggregate_light_separator_lockin_round.py"),
        "--round-root",
        str(round_root),
        "--run-records",
    ...
    if agg_rc != 0:
        raise SystemExit(f"aggregate_light_separator_lockin_round failed with exit code {agg_rc}")
