...
OUT_ROOT="${OUT_ROOT:-outputs/phase6x_early_collapse_recovery_n200_clean}"
...
# Backward-compatible mirror for tools expecting shared_sample_ids.
...
manifest = {
    "phase": "phase6x_early_collapse_recovery_n200_clean",
...
echo "=== summarize PHASE6X early-collapse recovery n200 clean ==="
PYTHONPATH=. "${PYTHON}" scripts/summarize_phase6x_early_collapse_recovery.py \
  ...
  --top-k 20 \
  --summary-md-name "PHASE6X_EARLY_COLLAPSE_RECOVERY_N200_CLEAN_SUMMARY.md" \
  --summary-json-name "phase6x_early_collapse_recovery_n200_clean_summary.json" \
...
echo "=== done ==="
echo "Summary: ${OUT_ROOT}/PHASE6X_EARLY_COLLAPSE_RECOVERY_N200_CLEAN_SUMMARY.md"
echo "Summary JSON: ${OUT_ROOT}/phase6x_early_collapse_recovery_n200_clean_summary.json"
