...
if [[ -n "${RESUME_ROUND_ROOT:-}" ]]; then
  ...
  ROUND_ROOT="outputs/overnight_guarded_answer_preserve/${RUN_STAMP}"
fi

LOG_ROOT="logs/overnight_guarded_answer_preserve/$(basename "${ROUND_ROOT}")"
...
maybe_snapshot_git_state() {
  ...
  if [[ "${ENABLE_GIT_SNAPSHOT:-false}" != "true" ]]; then
    ...
    log_msg "[INFO]   git commit -m 'chore: snapshot before guarded answer-preserve refinement'"
    log_msg "[INFO]   git checkout -b guarded-answer-preserve-v1"
    return 0
  fi

  git add -A
  if ! git diff --cached --quiet; then
    git commit -m "chore: snapshot before guarded answer-preserve refinement"
  else
    log_msg "[INFO] No staged changes to commit before redesign branch."
  fi
  git checkout -b guarded-answer-preserve-v1 2>/dev/null || git checkout guarded-answer-preserve-v1
...
lines = [
    '# Overnight Guarded Answer-Preserve Metrics (RAG full)',
    '',
    '| stage | variant | dataset | R@5 | EM | F1 | preserve_activation | preserve_usefulness | anchor_bridge_balance | bridge_purity | answer_side_density | support_set_compactness | bridge_noise_ratio | bridge_to_answer_path_hit | conversion_after_bridge | conversion_after_preserve | hotpot_overpreserve_rate | err_bridge_missing | err_answer_missing | err_bridge_noisy | err_gen_fail | oracle_gap | gap_to_hippo_r5 | gap_to_hippo_f1 |',
    '|---|---|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|',
]
for r in rows:
    lines.append(
        f"| {r['stage']} | {r['variant']} | {r['dataset']} | {r['Recall@5']:.4f} | {r['EM']:.4f} | {r['F1']:.4f} | {r['answer_preserve_activation_rate']:.4f} | {r['preserved_answer_usefulness']:.4f} | {r['anchor_bridge_balance']:.4f} | {r['bridge_purity']:.4f} | {r['answer_side_density']:.4f} | {r['support_set_compactness']:.4f} | {r['bridge_noise_ratio']:.4f} | {r['bridge_to_answer_path_hit']:.4f} | {r['conversion_after_bridge']:.4f} | {r['conversion_after_preserve']:.4f} | {r['hotpot_overpreserve_rate']:.4f} | {r['error_bucket_bridge_missing']:.4f} | {r['error_bucket_answer_side_missing']:.4f} | {r['error_bucket_bridge_present_noisy']:.4f} | {r['error_bucket_answer_present_generation_fail']:.4f} | {r['oracle_gap']:+.4f} | {r['gap_to_hippo_r5']:+.4f} | {r['gap_to_hippo_f1']:+.4f} |"
    )
Path(out_md).write_text('\n'.join(lines) + '\n', encoding='utf-8')
PY
}
...
payload = {
    ...
    "fixed_by": "manual_freeze_for_guarded_answer_preserve_round",
...
log_msg "[Stage P1] p3_answer_preserve_base"
run_variant_for_dataset "p1" "p3_answer_preserve_base" "hotpotqa" "hotpot_f2" "${UNIVERSAL_REGIME}" "p3_answer_preserve_base" "inherit" 0 0 "${LIMIT_RAG}"
run_variant_for_dataset "p1" "p3_answer_preserve_base" "2wikimultihopqa" "${WIKI_CORE_PROFILE}" "${UNIVERSAL_REGIME}" "p3_answer_preserve_base" "inherit" 0 0 "${LIMIT_RAG}"

log_msg "[Stage P2] p3_answer_preserve_guarded_hotpot"
run_variant_for_dataset "p2" "p3_answer_preserve_guarded_hotpot" "hotpotqa" "hotpot_f2" "${UNIVERSAL_REGIME}" "p3_answer_preserve_guarded_hotpot" "inherit" 0 0 "${LIMIT_RAG}"
run_variant_for_dataset "p2" "p3_answer_preserve_guarded_hotpot" "2wikimultihopqa" "${WIKI_CORE_PROFILE}" "${UNIVERSAL_REGIME}" "p3_answer_preserve_guarded_hotpot" "inherit" 0 0 "${LIMIT_RAG}"

log_msg "[Stage P3] p3_answer_preserve_confidence_gated"
run_variant_for_dataset "p3" "p3_answer_preserve_confidence_gated" "hotpotqa" "hotpot_f2" "${UNIVERSAL_REGIME}" "p3_answer_preserve_confidence_gated" "inherit" 0 0 "${LIMIT_RAG}"
run_variant_for_dataset "p3" "p3_answer_preserve_confidence_gated" "2wikimultihopqa" "${WIKI_CORE_PROFILE}" "${UNIVERSAL_REGIME}" "p3_answer_preserve_confidence_gated" "inherit" 0 0 "${LIMIT_RAG}"
