...
# Universal embedding regimes (single global winner selected in Stage U0).
U0_OLD_MAX_LEN="${U0_OLD_MAX_LEN:-192}"
U0_OLD_MAX_CHARS="${U0_OLD_MAX_CHARS:-600}"
U0_MID_MAX_LEN="${U0_MID_MAX_LEN:-256}"
U0_MID_MAX_CHARS="${U0_MID_MAX_CHARS:-800}"
U0_NEW_MAX_LEN="${U0_NEW_MAX_LEN:-320}"
U0_NEW_MAX_CHARS="${U0_NEW_MAX_CHARS:-900}"
...
if [[ -n "${RESUME_ROUND_ROOT:-}" ]]; then
  ...
  ROUND_ROOT="outputs/overnight_universal_retrieval_redesign/${RUN_STAMP}"
fi

LOG_ROOT="logs/overnight_universal_retrieval_redesign/$(basename "${ROUND_ROOT}")"
...
maybe_snapshot_git_state() {
  ...
  if [[ "${ENABLE_GIT_SNAPSHOT:-false}" != "true" ]]; then
    ...
    log_msg "[INFO]   git commit -m 'chore: snapshot before universal retrieval redesign'"
    log_msg "[INFO]   git checkout -b retrieval-redesign-universal-v1"
    return 0
  fi

  git add -A
  if ! git diff --cached --quiet; then
    git commit -m "chore: snapshot before universal retrieval redesign"
  else
    log_msg "[INFO] No staged changes to commit before redesign branch."
  fi
  git checkout -b retrieval-redesign-universal-v1 2>/dev/null || git checkout retrieval-redesign-universal-v1
  log_msg "[INFO] Active git branch: $(git rev-parse --abbrev-ref HEAD)"
}
...
if mode in {'hybrid_anchor_recall', 'bridge_coverage_full'}:
    cfg['hybrid_anchor_recall_enabled'] = True
if mode in {'bridge_candidate_induction', 'bridge_coverage_full'}:
    cfg['bridge_candidate_induction_enabled'] = True
if mode in {'role_aware_chunk_scoring', 'bridge_coverage_full'}:
    cfg['role_aware_chunk_scoring_enabled'] = True
if mode in {'coverage_selection', 'bridge_coverage_full'}:
    cfg['coverage_selection_enabled'] = True
...
lines = [
    '# Overnight Universal Retrieval Redesign Metrics (RAG full)',
    '',
...
select_universal_regime() {
  "${PYTHON_BIN}" - "${RECORD_TSV}" "${REF_BUNDLE_JSON}" "${UNIVERSAL_SELECTION_JSON}" <<'PY'
import json
import sys
from pathlib import Path

record_tsv, ref_json, out_json = sys.argv[1:4]
refs = json.loads(Path(ref_json).read_text(encoding="utf-8"))
...
def _r_at(summary, k):
    v = _f(summary.get(f"supporting_fact_recall_at_{k}", 0.0))
    if v > 0.0:
        return v
    s = summary.get("supporting_fact_recall_at_k", {}) or {}
    if str(k) in s:
...
raw = {r: {} for r in regime_map.values()}
...
with Path(record_tsv).open("r", encoding="utf-8") as f:
    ...
    for line in f:
        ...
        if len(parts) != 8:
            continue
        stage, variant, dataset, profile, kind, summary_path, log_path, status = parts
        if stage != "u0" or kind != "rag" or status not in {"ok", "skipped"}:
            continue
        regime = regime_map.get(str(variant))
        if not regime:
            continue
        sp = Path(summary_path)
        if not sp.exists():
        ...
        try:
            ...
            continue
        d2_f1 = _f(((refs.get(dataset, {}) or {}).get("d2", {}) or {}).get("f1", 0.0))
        hippo_r5 = _f(((refs.get(dataset, {}) or {}).get("hipporag2", {}) or {}).get("r5", 0.0))
        r5 = _r_at(summary, 5)
        f1 = _f(summary.get("f1", summary.get("F1", 0.0)))
        r5_rel = (r5 / hippo_r5) if hippo_r5 > 0 else 0.0
        f1_rel = (f1 / d2_f1) if d2_f1 > 0 else 0.0
        score = 0.5 * r5_rel + 0.5 * f1_rel
        raw[regime][dataset] = {
...
regime_scores = {}
for regime in ("old", "mid", "new"):
    ds = raw.get(regime, {})
    if "hotpotqa" not in ds or "2wikimultihopqa" not in ds:
        regime_scores[regime] = {"complete": False, "J": float("-inf")}
        continue
    j = 0.5 * (float(ds["hotpotqa"]["score"]) + float(ds["2wikimultihopqa"]["score"]))
    regime_scores[regime] = {"complete": True, "J": float(j)}

winner = "new"
winner_score = float("-inf")
for regime in ("old", "mid", "new"):
    rs = regime_scores.get(regime, {}) or {}
    if not bool(rs.get("complete", False)):
        continue
    j = float(rs.get("J", float("-inf")))
    if j > winner_score:
        winner = regime
        winner_score = j
...
payload = {
    ...
    "winner_J": float(winner_score if winner_score != float("-inf") else 0.0),
...
log_msg "[INFO] embedding_regime_old=max_length:${U0_OLD_MAX_LEN},max_chars:${U0_OLD_MAX_CHARS}"
log_msg "[INFO] embedding_regime_mid=max_length:${U0_MID_MAX_LEN},max_chars:${U0_MID_MAX_CHARS}"
log_msg "[INFO] embedding_regime_new=max_length:${U0_NEW_MAX_LEN},max_chars:${U0_NEW_MAX_CHARS}"
...
log_msg "[Stage U0] Universal regime freeze baselines (old/mid/new)"
for regime in old mid new; do
  log_msg "[U0] baseline_${regime} hotpot"
  run_variant_for_dataset "u0" "baseline_${regime}" "hotpotqa" "hotpot_f2" "${regime}" "baseline" "hotpot_fixed" 4 1 "${LIMIT_RAG}"
  log_msg "[U0] baseline_${regime} 2wiki"
  run_variant_for_dataset "u0" "baseline_${regime}" "2wikimultihopqa" "${WIKI_CORE_PROFILE}" "${regime}" "baseline" "wiki_fixed" 3 1 "${LIMIT_RAG}"
done

UNIVERSAL_REGIME="$(select_universal_regime)"
if [[ "${UNIVERSAL_REGIME}" != "old" && "${UNIVERSAL_REGIME}" != "mid" && "${UNIVERSAL_REGIME}" != "new" ]]; then
  UNIVERSAL_REGIME="new"
fi
log_msg "[Stage U0] selected universal embedding regime=${UNIVERSAL_REGIME} (details: ${UNIVERSAL_SELECTION_JSON})"

log_msg "[Stage U1] Retrieval objective redesign under universal regime=${UNIVERSAL_REGIME}"
for mode in \
  "hybrid_anchor_recall:r1_hybrid_anchor_recall" \
  "bridge_candidate_induction:r2_bridge_candidate_induction" \
  "role_aware_chunk_scoring:r3_role_aware_chunk_scoring" \
  "coverage_selection:r4_coverage_selection" \
  "bridge_coverage_full:r4_plus_fixed_downstream"
do
  retrieval_mode="${mode%%:*}"
  variant="${mode##*:}"
  log_msg "[U1] ${variant} hotpot"
  run_variant_for_dataset "u1" "${variant}" "hotpotqa" "hotpot_f2" "${UNIVERSAL_REGIME}" "${retrieval_mode}" "hotpot_fixed" 4 1 "${LIMIT_RAG}"
  log_msg "[U1] ${variant} 2wiki"
  run_variant_for_dataset "u1" "${variant}" "2wikimultihopqa" "${WIKI_CORE_PROFILE}" "${UNIVERSAL_REGIME}" "${retrieval_mode}" "wiki_fixed" 3 1 "${LIMIT_RAG}"
done
...
log_msg "[DONE] universal regime selection: ${UNIVERSAL_SELECTION_JSON}"
