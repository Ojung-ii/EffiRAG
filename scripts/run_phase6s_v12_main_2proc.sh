...
OUT_ROOT="${OUT_ROOT:-outputs/phase6s_v12_main_n1000_2proc}"
SAMPLE_SIZE="${SAMPLE_SIZE:-1000}"
PROCESS_COUNT="${PROCESS_COUNT:-2}"
GPU_A="${GPU_A:-0}"
GPU_B="${GPU_B:-1}"
STRICT_VALIDATE="${STRICT_VALIDATE:-1}"
RUN_METHODOLOGY_AUDIT="${RUN_METHODOLOGY_AUDIT:-1}"
PYTHON="${PYTHON:-/home/ojungii/miniconda3/envs/effirag/bin/python}"

PROFILE="${PROFILE:-unified_acr_rcedr_v12}"
RUN_A_NAME="${RUN_A_NAME:-v12_hotpotqa}"
RUN_B_NAME="${RUN_B_NAME:-v12_2wikimultihopqa}"
...
opt_enabled="${PHASE6T_SCORE_ATTACHMENT_OPT:-}"
if [[ -z "${opt_enabled}" ]]; then
  case "$(echo "${PHASE6T_OPTIMIZED:-1}" | tr '[:upper:]' '[:lower:]')" in
    1|true|yes|on) opt_enabled="1" ;;
    *) opt_enabled="0" ;;
  esac
fi

echo "=== Phase-6S v12 main n1000 2-process run ==="
...
echo "PHASE6T_SCORE_ATTACHMENT_OPT=${opt_enabled}"

if [[ "${RUN_METHODOLOGY_AUDIT}" == "1" ]]; then
...
SCHEDULE_JSONL="${OUT_ROOT}/_phase6s_v12_main_scheduled_runs.jsonl"
: > "${SCHEDULE_JSONL}"
...
register_run() {
  ...
  local gpu="$3"
  local group="$4"
  printf '{"run_name":"%s","dataset":"%s","profile":"%s","gpu":"%s","process_group":"%s","role":"main","kind":"unified","scheduled":true}\n' \
    "${run_name}" "${dataset}" "${PROFILE}" "${gpu}" "${group}" >> "${SCHEDULE_JSONL}"
}

TASKS_A=("${PROFILE}|hotpotqa|${RUN_A_NAME}|${GPU_A}|A")
TASKS_B=("${PROFILE}|2wikimultihopqa|${RUN_B_NAME}|${GPU_B}|B")
...
register_run "hotpotqa" "${RUN_A_NAME}" "${GPU_A}" "A"
register_run "2wikimultihopqa" "${RUN_B_NAME}" "${GPU_B}" "B"
...
run_unified_qa_one_dataset() {
  ...
  PHASE6T_SCORE_ATTACHMENT_OPT="${opt_enabled}" \
...
echo "=== summarize Phase-6S v12 main 2proc ==="
PYTHONPATH=. "${PYTHON}" scripts/summarize_phase6s_v12_main_2proc.py \
  --out-root "${OUT_ROOT}" \
  2>&1 | tee "${OUT_ROOT}/logs/summarize_phase6s_v12_main_2proc.log"
...
import json
import os
path = os.path.join(os.environ["OUT_ROOT"], "phase6s_run_manifest.json")
if os.path.exists(path):
    print(len(json.load(open(path)).get("runs", [])))
else:
    print("missing phase6s_run_manifest.json")
PY
...
echo "=== done ==="
echo "Summary: ${OUT_ROOT}/PHASE6S_V12_MAIN_2PROC_SUMMARY.md"
echo "Summary JSON: ${OUT_ROOT}/phase6s_v12_main_2proc_summary.json"
