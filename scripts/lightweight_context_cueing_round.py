...
def _variant_plan(variant: str) -> Dict[str, Any]:
    ...
    if vv == "minimal_role_prefix":
        return {
            "render_mode": "corridor_aware_flat",
            "prompt_variant": "default",
            "order_flags": ["minimal_role_prefix"],
            "render_variant": "minimal_role_prefix",
            "prompt_variant_label": "default",
        }
    if vv == "inline_source_role_hint":
        return {
            "render_mode": "corridor_aware_flat",
            "prompt_variant": "default",
            "order_flags": ["inline_source_role_hint"],
            "render_variant": "inline_source_role_hint",
            "prompt_variant_label": "default",
        }
    if vv == "title_preserving_compact_render":
        return {
            "render_mode": "corridor_aware_flat",
            "prompt_variant": "default",
            "order_flags": ["title_preserving_compact_render"],
            "render_variant": "title_preserving_compact_render",
            "prompt_variant_label": "default",
        }
    ...
    if vv == "query_aware_soft_ordering":
        return {
            "render_mode": "corridor_aware_flat",
            "prompt_variant": "default",
            "order_flags": ["query_aware_soft_ordering"],
            "render_variant": "query_aware_soft_ordering",
            "prompt_variant_label": "default",
        }
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        "--variants",
        default="champion_reconfirm,minimal_role_prefix,inline_source_role_hint,title_preserving_compact_render,light_separator_render,query_aware_soft_ordering",
    )
...
def main() -> None:
    ...
    retrieval_fixed_manifest: Dict[str, Any] = {
        "generated_at_utc": datetime.now(timezone.utc).isoformat(),
        "champion_root": str(champion_root),
        "retrieval_fixed": True,
        "datasets": [],
    }
    for ds in datasets:
        row = champion_by_dataset[ds]
        summary_path = Path(_strip(row.get("summary_path"))).resolve()
        query_path = summary_path.with_name("rag_query_results.jsonl")
        if not query_path.exists():
            raise SystemExit(f"missing champion query artifact for dataset={ds}: {query_path}")
    ...
    for ds in datasets:
        ...
        champion_query_path = champion_summary_path.with_name("rag_query_results.jsonl")
        if not champion_query_path.exists():
            raise SystemExit(f"missing champion query artifact for dataset={ds}: {champion_query_path}")
        ...
        for vv in variants:
            ...
            profile = "champion_reconfirm" if vv == "champion_reconfirm" else "lightweight_context_cueing"
            ...
            cfg["precomputed_retrieval_path"] = str(champion_query_path)
                ...
                run_records.append(
                    {
                        ...
                        "precomputed_retrieval_path": str(champion_query_path),
            ...
            with log_path.open("ab") as lf:
                header = (
                    ...
                    f"[PRECOMPUTED_RETRIEVAL] {champion_query_path}\n"
                    f"[RETRIEVAL_FIXED] true\n"
                    f"[FLAGS] {flags}\n"
    ...
    agg_cmd = [
        sys.executable,
        str(REPO_ROOT / "scripts" / "aggregate_lightweight_context_cueing_round.py"),
        "--round-root",
        str(round_root),
        "--run-records",
    ...
    if agg_rc != 0:
        raise SystemExit(f"aggregate_lightweight_context_cueing_round failed with exit code {agg_rc}")
