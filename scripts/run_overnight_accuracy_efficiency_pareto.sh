...
if [[ -n "${RESUME_ROUND_ROOT:-}" ]]; then
  ...
  ROUND_ROOT="outputs/overnight_accuracy_efficiency_pareto/${RUN_STAMP}"
fi

LOG_ROOT="logs/overnight_accuracy_efficiency_pareto/$(basename "${ROUND_ROOT}")"
...
maybe_snapshot_git_state() {
  ...
  if [[ "${ENABLE_GIT_SNAPSHOT:-false}" != "true" ]]; then
    ...
    log_msg "[INFO]   git commit -m 'chore: snapshot before accuracy-efficiency pareto round'"
    return 0
  fi

  git add -A
  if ! git diff --cached --quiet; then
    git commit -m "chore: snapshot before accuracy-efficiency pareto round"
  else
...
lines = [
    '# Overnight Accuracy-Efficiency Pareto Metrics (RAG full)',
    '',
    '## Main Performance Table',
    '',
    '| stage | variant | dataset | R@5 | EM | F1 | retrieval_ms | total_ms | gap_to_hippo_r5 | oracle_gap |',
    '|---|---|---|---:|---:|---:|---:|---:|---:|---:|',
]
for r in rows:
    lines.append(
        f"| {r['stage']} | {r['variant']} | {r['dataset']} | {r['Recall@5']:.4f} | {r['EM']:.4f} | {r['F1']:.4f} | {r['retrieval_ms']:.2f} | {r['total_ms']:.2f} | {r['gap_to_hippo_r5']:+.4f} | {r['oracle_gap']:+.4f} |"
    )
...
lines += [
    ...
    '## Efficiency Table',
    '',
    '| stage | variant | dataset | ΔR@5_vs_baseline | ΔEM_vs_baseline | ΔF1_vs_baseline | Δretrieval_ms_vs_baseline | Δtotal_ms_vs_baseline | F1_per_100ms | R5_per_100ms | oracle_gap_per_ms |',
    '|---|---|---|---:|---:|---:|---:|---:|---:|---:|---:|',
]
for r in rows:
    if str(r.get('stage')) == 'reference':
        continue
    lines.append(
        f"| {r['stage']} | {r['variant']} | {r['dataset']} | {r['delta_R5_vs_baseline']:+.4f} | {r['delta_EM_vs_baseline']:+.4f} | {r['delta_F1_vs_baseline']:+.4f} | {r['delta_retrieval_ms_vs_baseline']:+.2f} | {r['delta_total_ms_vs_baseline']:+.2f} | {r['f1_per_100ms']:.4f} | {r['r5_per_100ms']:.4f} | {r['oracle_gap_per_ms']:.6f} |"
    )
...
lines += [
    ...
    '## Diagnostic Table',
    '',
    '| stage | variant | dataset | path_complete_rate | incomplete_path_rate | chain_compactness | answer_present_but_generation_fail | conversion_after_path_preserve |',
    '|---|---|---|---:|---:|---:|---:|---:|',
]
for r in rows:
    if str(r.get('stage')) == 'reference':
        continue
    lines.append(
        f"| {r['stage']} | {r['variant']} | {r['dataset']} | {r['path_complete_rate']:.4f} | {r['incomplete_path_rate']:.4f} | {r['chain_compactness']:.4f} | {r['error_bucket_answer_present_generation_fail']:.4f} | {r['conversion_after_path_preserve']:.4f} |"
    )
...
payload = {
    ...
    "fixed_by": "manual_freeze_for_accuracy_efficiency_pareto_round",
...
log_msg "[Stage P2] r2_plus_path_preserve"
run_variant_for_dataset "p2" "r2_plus_path_preserve" "hotpotqa" "hotpot_f2" "${UNIVERSAL_REGIME}" "r2_plus_path_preserve" "inherit" 0 0 "${LIMIT_RAG}"
run_variant_for_dataset "p2" "r2_plus_path_preserve" "2wikimultihopqa" "${WIKI_CORE_PROFILE}" "${UNIVERSAL_REGIME}" "r2_plus_path_preserve" "inherit" 0 0 "${LIMIT_RAG}"

log_msg "[Stage P3] r2_plus_path_preserve_guarded"
run_variant_for_dataset "p3" "r2_plus_path_preserve_guarded" "hotpotqa" "hotpot_f2" "${UNIVERSAL_REGIME}" "r2_plus_path_preserve_guarded" "inherit" 0 0 "${LIMIT_RAG}"
run_variant_for_dataset "p3" "r2_plus_path_preserve_guarded" "2wikimultihopqa" "${WIKI_CORE_PROFILE}" "${UNIVERSAL_REGIME}" "r2_plus_path_preserve_guarded" "inherit" 0 0 "${LIMIT_RAG}"

log_msg "[Stage P4] r2_plus_path_preserve_compact_lite"
run_variant_for_dataset "p4" "r2_plus_path_preserve_compact_lite" "hotpotqa" "hotpot_f2" "${UNIVERSAL_REGIME}" "r2_plus_path_preserve_compact_lite" "inherit" 0 0 "${LIMIT_RAG}"
run_variant_for_dataset "p4" "r2_plus_path_preserve_compact_lite" "2wikimultihopqa" "${WIKI_CORE_PROFILE}" "${UNIVERSAL_REGIME}" "r2_plus_path_preserve_compact_lite" "inherit" 0 0 "${LIMIT_RAG}"
