...
if [[ -n "${RESUME_ROUND_ROOT:-}" ]]; then
  ...
  ROUND_ROOT="outputs/overnight_pamae_connector_round/${RUN_STAMP}"
fi

LOG_ROOT="logs/overnight_pamae_connector_round/$(basename "${ROUND_ROOT}")"
...
maybe_snapshot_git_state() {
  ...
  if [[ "${ENABLE_GIT_SNAPSHOT:-false}" != "true" ]]; then
    ...
    log_msg "[INFO]   git commit -m 'chore: snapshot before PAMAE-style retrieval redesign'"
    log_msg "[INFO]   git checkout -b pamae-connector-retrieval-v1"
    return 0
  fi

  git add -A
  if ! git diff --cached --quiet; then
    git commit -m "chore: snapshot before PAMAE-style retrieval redesign"
  else
    log_msg "[INFO] No staged changes to commit before redesign branch."
  fi
  git checkout -b pamae-connector-retrieval-v1 2>/dev/null || git checkout pamae-connector-retrieval-v1
...
if mode in {'corridor_role_constrained', 'bridge_aware_run_plus_role_constrained_corridor'}:
...
if tag == 'run_objective_bridge_aware':
    cfg['run_score_semantic_weight'] = 0.22
    cfg['run_score_anchor_weight'] = 0.18
    cfg['run_score_structure_weight'] = 0.22
    cfg['run_score_bridge_weight'] = 0.22
    cfg['run_score_redundancy_weight'] = 0.08
    cfg['run_score_pair_coverage_weight'] = 0.04
    cfg['run_score_bridge_completeness_weight'] = 0.04
    cfg['seed_objective_bridge_weight'] = 0.28
elif tag == 'run_objective_role_balanced':
    cfg['run_score_semantic_weight'] = 0.20
    cfg['run_score_anchor_weight'] = 0.20
    cfg['run_score_structure_weight'] = 0.20
    cfg['run_score_bridge_weight'] = 0.16
    cfg['run_score_redundancy_weight'] = 0.08
    cfg['run_score_pair_coverage_weight'] = 0.08
    cfg['run_score_bridge_completeness_weight'] = 0.05
    cfg['run_score_entity_chunk_grounding_weight'] = 0.03
    cfg['run_score_anchor_dispersion_penalty'] = 0.08
    cfg['seed_objective_bridge_weight'] = 0.24
    cfg['seed_objective_anchor_coverage_weight'] = 0.20
elif tag == 'corridor_role_constrained':
    cfg['coverage_selection_enabled'] = True
    cfg['coverage_selection_role_weight'] = 0.55
    cfg['coverage_selection_redundancy_weight'] = 0.22
elif tag == 'bridge_aware_run_plus_role_constrained_corridor':
    cfg['run_score_semantic_weight'] = 0.22
    cfg['run_score_anchor_weight'] = 0.18
    cfg['run_score_structure_weight'] = 0.22
    cfg['run_score_bridge_weight'] = 0.22
...
def _row(stage, variant, dataset, summary_path, summary):
    ...
    return {
        ...
        'seed_anchor_recall': _f(summary.get('seed_anchor_recall', 0.0)),
        'seed_bridge_recall': _f(summary.get('seed_bridge_recall', 0.0)),
        'seed_answer_recall': _f(summary.get('seed_answer_recall', 0.0)),
        'run_bridge_coverage': _f(summary.get('run_bridge_coverage', 0.0)),
        'corridor_role_coverage': _f(summary.get('corridor_role_coverage', 0.0)),
        'seed_diversity': _f(summary.get('seed_diversity', 0.0)),
        'seed_redundancy': _f(summary.get('seed_redundancy', 0.0)),
        'redundancy_rate': _f(summary.get('redundancy_rate', 0.0)),
...
lines = [
    '# Overnight PAMAE-Connector Retrieval Metrics (RAG full)',
    '',
    '| stage | variant | dataset | R@5 | EM | F1 | seed_bridge_recall | run_bridge_coverage | corridor_role_coverage | oracle_gap | gap_to_hippo_r5 | gap_to_hippo_f1 |',
    '|---|---|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|',
]
for r in rows:
    lines.append(
        f"| {r['stage']} | {r['variant']} | {r['dataset']} | {r['Recall@5']:.4f} | {r['EM']:.4f} | {r['F1']:.4f} | {r['seed_bridge_recall']:.4f} | {r['run_bridge_coverage']:.4f} | {r['corridor_role_coverage']:.4f} | {r['oracle_gap']:+.4f} | {r['gap_to_hippo_r5']:+.4f} | {r['gap_to_hippo_f1']:+.4f} |"
    )
Path(out_md).write_text('\n'.join(lines) + '\n', encoding='utf-8')
PY
}
...
payload = {
    ...
    "fixed_by": "manual_freeze_for_pamae_connector_round",
...
run_variant_for_dataset "p0" "baseline_mid_reconfirm" "hotpotqa" "hotpot_f2" "${UNIVERSAL_REGIME}" "baseline" "hotpot_fixed" 4 1 "${LIMIT_RAG}"
run_variant_for_dataset "p0" "baseline_mid_reconfirm" "2wikimultihopqa" "${WIKI_CORE_PROFILE}" "${UNIVERSAL_REGIME}" "baseline" "wiki_fixed" 3 1 "${LIMIT_RAG}"

log_msg "[Stage P1] seed_quality_analysis"
run_variant_for_dataset "p1" "seed_quality_analysis" "hotpotqa" "hotpot_f2" "${UNIVERSAL_REGIME}" "seed_quality_analysis" "hotpot_fixed" 4 1 "${LIMIT_RAG}"
run_variant_for_dataset "p1" "seed_quality_analysis" "2wikimultihopqa" "${WIKI_CORE_PROFILE}" "${UNIVERSAL_REGIME}" "seed_quality_analysis" "wiki_fixed" 3 1 "${LIMIT_RAG}"

log_msg "[Stage P2] run selection redesign"
for variant in \
  "run_objective_bridge_aware" \
  "run_objective_role_balanced"
do
  log_msg "[P2] ${variant} hotpot"
  run_variant_for_dataset "p2" "${variant}" "hotpotqa" "hotpot_f2" "${UNIVERSAL_REGIME}" "${variant}" "hotpot_fixed" 4 1 "${LIMIT_RAG}"
  log_msg "[P2] ${variant} 2wiki"
  run_variant_for_dataset "p2" "${variant}" "2wikimultihopqa" "${WIKI_CORE_PROFILE}" "${UNIVERSAL_REGIME}" "${variant}" "wiki_fixed" 3 1 "${LIMIT_RAG}"
done

log_msg "[Stage P3] corridor selection redesign"
for variant in \
  "corridor_role_constrained" \
  "bridge_aware_run_plus_role_constrained_corridor"
do
  log_msg "[P3] ${variant} hotpot"
  run_variant_for_dataset "p3" "${variant}" "hotpotqa" "hotpot_f2" "${UNIVERSAL_REGIME}" "${variant}" "hotpot_fixed" 4 1 "${LIMIT_RAG}"
  log_msg "[P3] ${variant} 2wiki"
  run_variant_for_dataset "p3" "${variant}" "2wikimultihopqa" "${WIKI_CORE_PROFILE}" "${UNIVERSAL_REGIME}" "${variant}" "wiki_fixed" 3 1 "${LIMIT_RAG}"
done
