...
OUT_ROOT="${OUT_ROOT:-outputs/phase6t_proposal_trace_hotpotqa_n5}"
...
if [[ "${PROCESS_COUNT}" != "1" ]]; then
  echo "[ERROR] PROCESS_COUNT must be 1 for sequential trace profiling." >&2
  exit 1
fi
...
echo "=== Phase-6T proposal sequential trace (GPU1-only) ==="
...
echo "PHASE6T_PROP_TRACE_LIMIT=${PHASE6T_PROP_TRACE_LIMIT}"
echo "PHASE6T_PROP_TRACE_SAMPLE_IDS=${PHASE6T_PROP_TRACE_SAMPLE_IDS}"
echo "PHASE6T_PROP_TRACE_LOG=${PHASE6T_PROP_TRACE_LOG}"
...
SCHEDULE_JSONL="${OUT_ROOT}/_phase6t_proposal_trace_scheduled_runs.jsonl"
: > "${SCHEDULE_JSONL}"
printf '{"run_name":"%s","dataset":"%s","profile":"%s","gpu":"%s","process_group":"A","role":"proposal_trace_profile","kind":"unified","scheduled":true}\n' \
...
echo "=== summarize proposal trace ==="
PYTHONPATH=. "${PYTHON}" scripts/summarize_phase6t_proposal_trace.py \
  --trace-log "${OUT_ROOT}/logs/proposal_trace.log" \
  --out-root "${OUT_ROOT}" \
  2>&1 | tee "${OUT_ROOT}/logs/summarize_phase6t_proposal_trace.log"
...
echo "=== done ==="
echo "Trace log: ${OUT_ROOT}/logs/proposal_trace.log"
echo "Summary: ${OUT_ROOT}/PHASE6T_PROPOSAL_TRACE_SUMMARY.md"
echo "JSON: ${OUT_ROOT}/proposal_trace_summary.json"
