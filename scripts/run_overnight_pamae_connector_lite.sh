...
if [[ -n "${RESUME_ROUND_ROOT:-}" ]]; then
  ...
  ROUND_ROOT="outputs/overnight_pamae_connector_lite/${RUN_STAMP}"
fi

LOG_ROOT="logs/overnight_pamae_connector_lite/$(basename "${ROUND_ROOT}")"
...
maybe_snapshot_git_state() {
  ...
  if [[ "${ENABLE_GIT_SNAPSHOT:-false}" != "true" ]]; then
    ...
    log_msg "[INFO]   git commit -m 'chore: snapshot before PAMAE connector refinement lite'"
    log_msg "[INFO]   git checkout -b pamae-connector-lite-v1"
    return 0
  fi

  git add -A
  if ! git diff --cached --quiet; then
    git commit -m "chore: snapshot before PAMAE connector refinement lite"
  else
    log_msg "[INFO] No staged changes to commit before redesign branch."
  fi
  git checkout -b pamae-connector-lite-v1 2>/dev/null || git checkout pamae-connector-lite-v1
...
lines = [
    '# Overnight PAMAE-Connector Lite Metrics (RAG full)',
    '',
    '| stage | variant | dataset | R@5 | EM | F1 | useful_bridge_rate | bridge_to_answer_path_hit | answer_conversion | f1_per_r5 | err_bridge_missing | err_answer_missing | err_bridge_noisy | err_gen_fail | oracle_gap | gap_to_hippo_r5 | gap_to_hippo_f1 |',
    '|---|---|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|',
]
for r in rows:
    lines.append(
        f"| {r['stage']} | {r['variant']} | {r['dataset']} | {r['Recall@5']:.4f} | {r['EM']:.4f} | {r['F1']:.4f} | {r['useful_bridge_rate']:.4f} | {r['bridge_to_answer_path_hit']:.4f} | {r['answer_conversion']:.4f} | {r['f1_per_r5']:.4f} | {r['error_bucket_bridge_missing']:.4f} | {r['error_bucket_answer_side_missing']:.4f} | {r['error_bucket_bridge_present_noisy']:.4f} | {r['error_bucket_answer_present_generation_fail']:.4f} | {r['oracle_gap']:+.4f} | {r['gap_to_hippo_r5']:+.4f} | {r['gap_to_hippo_f1']:+.4f} |"
    )
Path(out_md).write_text('\n'.join(lines) + '\n', encoding='utf-8')
PY
}
...
payload = {
    ...
    "fixed_by": "manual_freeze_for_pamae_connector_lite_round",
...
log_msg "[Stage L0] baseline_mid_reconfirm"
run_variant_for_dataset "l0" "baseline_mid_reconfirm" "hotpotqa" "hotpot_f2" "${UNIVERSAL_REGIME}" "baseline" "hotpot_fixed" 4 1 "${LIMIT_RAG}"
run_variant_for_dataset "l0" "baseline_mid_reconfirm" "2wikimultihopqa" "${WIKI_CORE_PROFILE}" "${UNIVERSAL_REGIME}" "baseline" "wiki_fixed" 3 1 "${LIMIT_RAG}"

log_msg "[Stage L1] r2_bridge_only"
run_variant_for_dataset "l1" "r2_bridge_only" "hotpotqa" "hotpot_f2" "${UNIVERSAL_REGIME}" "r2_bridge_only" "hotpot_fixed" 4 1 "${LIMIT_RAG}"
run_variant_for_dataset "l1" "r2_bridge_only" "2wikimultihopqa" "${WIKI_CORE_PROFILE}" "${UNIVERSAL_REGIME}" "r2_bridge_only" "wiki_fixed" 3 1 "${LIMIT_RAG}"

log_msg "[Stage L2] r2_plus_r3_anchor_light"
run_variant_for_dataset "l2" "r2_plus_r3_anchor_light" "hotpotqa" "hotpot_f2" "${UNIVERSAL_REGIME}" "r2_plus_r3_anchor_light" "hotpot_fixed" 4 1 "${LIMIT_RAG}"
run_variant_for_dataset "l2" "r2_plus_r3_anchor_light" "2wikimultihopqa" "${WIKI_CORE_PROFILE}" "${UNIVERSAL_REGIME}" "r2_plus_r3_anchor_light" "wiki_fixed" 3 1 "${LIMIT_RAG}"

log_msg "[Stage L3] r2_plus_r3_answer_light"
run_variant_for_dataset "l3" "r2_plus_r3_answer_light" "hotpotqa" "hotpot_f2" "${UNIVERSAL_REGIME}" "r2_plus_r3_answer_light" "hotpot_fixed" 4 1 "${LIMIT_RAG}"
run_variant_for_dataset "l3" "r2_plus_r3_answer_light" "2wikimultihopqa" "${WIKI_CORE_PROFILE}" "${UNIVERSAL_REGIME}" "r2_plus_r3_answer_light" "wiki_fixed" 3 1 "${LIMIT_RAG}"
