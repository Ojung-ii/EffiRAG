...
OUT_ROOT="${OUT_ROOT:-outputs/phase6t_latency_isolation_hotpotqa_n20_gpu1_2proc}"
...
echo "=== Phase-6T latency bottleneck isolation (GPU1-only) ==="
...
echo "=== Phase-6T latency isolation audit gate ==="
PYTHONPATH=. "${PYTHON}" scripts/audit_phase6t_latency_triage.py \
  ...
  --out-root "${OUT_ROOT}" \
  2>&1 | tee "${OUT_ROOT}/logs/audit_phase6t_latency_isolation.log"

SCHEDULE_JSONL="${OUT_ROOT}/_phase6t_latency_isolation_scheduled_runs.jsonl"
: > "${SCHEDULE_JSONL}"
...
register_run () {
  ...
  local run_name="$3"
  local group="$4"
  printf '{"run_name":"%s","dataset":"%s","profile":"%s","gpu":"%s","process_group":"%s","role":"latency_isolation","kind":"unified","scheduled":true}\n' \
...
if [[ "${PROCESS_COUNT}" == "2" ]]; then
  TASKS_A=(
    ...
    "unified_acr_rcedr_v12_sota_contract_fast_rerank5|${DATASET}|fast_rerank5_${DATASET}|A"
  )
  TASKS_B=(
    "unified_acr_rcedr_v12_sota_contract_fast_no_sentence_rerank|${DATASET}|fast_no_sentence_rerank_${DATASET}|B"
    "unified_acr_rcedr_v12_sota_contract_fast_proposal_ultralight|${DATASET}|fast_proposal_ultralight_${DATASET}|B"
  )
else
  TASKS_A=(
    "unified_acr_rcedr_v12_sota_contract_fast|${DATASET}|fast_${DATASET}|A"
    ...
    "unified_acr_rcedr_v12_sota_contract_fast_rerank5|${DATASET}|fast_rerank5_${DATASET}|A"
    "unified_acr_rcedr_v12_sota_contract_fast_proposal_ultralight|${DATASET}|fast_proposal_ultralight_${DATASET}|A"
  )
fi
...
manifest = {
    "phase": "phase6t_latency_bottleneck_isolation",
...
echo "=== summarize Phase-6T latency isolation ==="
PYTHONPATH=. "${PYTHON}" scripts/summarize_phase6t_latency_isolation.py \
  --out-root "${OUT_ROOT}" \
  --dataset "${DATASET}" \
  2>&1 | tee "${OUT_ROOT}/logs/summarize_phase6t_latency_isolation.log"
...
echo "=== done ==="
echo "Summary: ${OUT_ROOT}/PHASE6T_LATENCY_ISOLATION_SUMMARY.md"
echo "Latency profile summary: ${OUT_ROOT}/PHASE6T_LATENCY_PROFILE_SUMMARY.md"
echo "Summary JSON: ${OUT_ROOT}/phase6t_latency_isolation_summary.json"
