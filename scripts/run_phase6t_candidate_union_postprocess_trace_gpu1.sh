...
OUT_ROOT="${OUT_ROOT:-outputs/phase6t_candidate_union_postprocess_trace_hotpotqa_n5}"
...
RUN_A_NAME="posttrace_diag_on_hotpotqa"
RUN_B_NAME="posttrace_diag_off_hotpotqa"

if [[ "${PROCESS_COUNT}" != "1" ]]; then
  echo "[ERROR] PROCESS_COUNT must be 1 for postprocess trace profiling." >&2
  exit 1
fi
...
export PHASE6T_CAND_UNION_POST_TRACE="${PHASE6T_CAND_UNION_POST_TRACE:-1}"
export PHASE6T_CAND_UNION_POST_TRACE_LIMIT="${PHASE6T_CAND_UNION_POST_TRACE_LIMIT:-5}"
export PHASE6T_CAND_UNION_POST_TRACE_SAMPLE_IDS="${PHASE6T_CAND_UNION_POST_TRACE_SAMPLE_IDS:-}"
export PHASE6T_CAND_UNION_POST_TRACE_LOG="${OUT_ROOT}/logs/candidate_union_postprocess_trace.log"
: > "${PHASE6T_CAND_UNION_POST_TRACE_LOG}"

echo "=== Phase-6T candidate-union outer postprocess trace (GPU1-only) ==="
...
echo "PHASE6T_CAND_UNION_POST_TRACE=${PHASE6T_CAND_UNION_POST_TRACE}"
echo "PHASE6T_CAND_UNION_POST_TRACE_LIMIT=${PHASE6T_CAND_UNION_POST_TRACE_LIMIT}"
echo "PHASE6T_CAND_UNION_POST_TRACE_SAMPLE_IDS=${PHASE6T_CAND_UNION_POST_TRACE_SAMPLE_IDS}"
echo "PHASE6T_CAND_UNION_POST_TRACE_LOG=${PHASE6T_CAND_UNION_POST_TRACE_LOG}"
...
SCHEDULE_JSONL="${OUT_ROOT}/_phase6t_candidate_union_postprocess_scheduled_runs.jsonl"
: > "${SCHEDULE_JSONL}"
printf '{"run_name":"%s","dataset":"%s","profile":"%s","gpu":"%s","process_group":"A","role":"postprocess_trace_diag_on","kind":"unified","scheduled":true}\n' \
  "${RUN_A_NAME}" "${DATASET}" "${PROFILE}" "${GPU}" >> "${SCHEDULE_JSONL}"
printf '{"run_name":"%s","dataset":"%s","profile":"%s","gpu":"%s","process_group":"A","role":"postprocess_trace_diag_off","kind":"unified","scheduled":true}\n' \
  "${RUN_B_NAME}" "${DATASET}" "${PROFILE}" "${GPU}" >> "${SCHEDULE_JSONL}"
...
run_one() {
  local run_name="$1"
  local diag_disable="$2"

  rm -rf "${OUT_ROOT}/qa_runs/${run_name}"
  mkdir -p "${OUT_ROOT}/qa_runs/${run_name}"

  echo "=== [GPU ${GPU}] profile=${PROFILE} dataset=${DATASET} run=${run_name} diag_disable=${diag_disable} ==="
  if [[ "${diag_disable}" == "1" ]]; then
    PHASE6T_DISABLE_RETRIEVAL_DIAGNOSTICS=1 \
    ...
    PHASE6T_DISABLE_RETRIEVAL_DIAGNOSTICS=0 \
    CUDA_VISIBLE_DEVICES="${GPU}" PYTHONPATH=. "${PYTHON}" \
      scripts/run_unified_config_4ds.py \
      --profile "${PROFILE}" \
      --datasets "${DATASET}" \
...
run_one "${RUN_A_NAME}" "0"
run_one "${RUN_B_NAME}" "1"

echo "=== summarize candidate-union postprocess trace ==="
PYTHONPATH=. "${PYTHON}" scripts/summarize_phase6t_candidate_union_postprocess_trace.py \
  --trace-log "${OUT_ROOT}/logs/candidate_union_postprocess_trace.log" \
  --out-root "${OUT_ROOT}" \
  2>&1 | tee "${OUT_ROOT}/logs/summarize_phase6t_candidate_union_postprocess_trace.log"
...
echo "=== done ==="
echo "Trace log: ${OUT_ROOT}/logs/candidate_union_postprocess_trace.log"
echo "Summary: ${OUT_ROOT}/PHASE6T_CANDIDATE_UNION_POSTPROCESS_TRACE_SUMMARY.md"
echo "JSON: ${OUT_ROOT}/candidate_union_postprocess_trace_summary.json"
