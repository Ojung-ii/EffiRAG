...
CALIBRATION_EMBEDDING_REGIME="${CALIBRATION_EMBEDDING_REGIME:-new}"
CALIBRATION_DELTA="${CALIBRATION_DELTA:-2}"
RESERVE_DELTA="${RESERVE_DELTA:-2}"
...
if [[ -n "${RESUME_ROUND_ROOT:-}" ]]; then
  ...
  ROUND_ROOT="outputs/overnight_regime_and_calibration/${RUN_STAMP}"
fi

LOG_ROOT="logs/overnight_regime_and_calibration/$(basename "${ROUND_ROOT}")"
...
create_variant_cfg() {
  ...
  local variant_tag="$4"
  local profile_key="$5"
  local embedding_regime="${6:-new}"
  local calibration_mode="${7:-off}"
  local downstream_mode="${8:-inherit}"
  local reorder_topk="${9:-3}"
  local pinning_min="${10:-1}"
  local calibration_delta="${11:-2}"
  local reserve_delta="${12:-2}"
...
# Embedding regime isolation (old vs new).
old_chars = int(knobs.get('embedding_text_max_chars', retr_cfg.get('embedding_text_max_chars', 600)) or 600)
old_len = int(knobs.get('embedding_max_length', retr_cfg.get('embedding_max_length', 192)) or 192)
new_exec = (bundle.get('execution_defaults', {}) or {})
new_chars = int(new_exec.get('embedding_text_max_chars', retr_cfg.get('embedding_text_max_chars', 900)) or 900)
new_len = int(new_exec.get('embedding_max_length', retr_cfg.get('embedding_max_length', 320)) or 320)
regime = str(embedding_regime or 'new').strip().lower()
if regime == 'old':
...
# Proposal expansion is intentionally disabled for this round.
cfg['shared_budget_profile'] = 'off'
cfg['shared_budget_proposal_step'] = 0
cfg['shared_budget_exploration_step'] = 0

e = max(1, int(cfg.get('semantic_topn_entity', knobs.get('semantic_topn_entity', 30)) or 30))
c = max(1, int(cfg.get('semantic_topn_chunk', knobs.get('semantic_topn_chunk', 15)) or 15))
r = max(1, int(cfg.get('graph_reserve_topn', knobs.get('graph_reserve_topn', 15)) or 15))
d = max(1, int(calibration_delta))
rd = max(1, int(reserve_delta))
cmode = str(calibration_mode or 'off').strip().lower()
if cmode == 'entity_up_chunk_down':
    e = e + d
    c = max(1, c - d)
elif cmode == 'entity_down_chunk_up':
    e = max(1, e - d)
    c = c + d
elif cmode == 'reserve_down':
    r = max(1, r - rd)
elif cmode == 'off':
    pass
else:
    raise RuntimeError(f'unsupported_calibration_mode:{calibration_mode}')
cfg['semantic_topn_entity'] = int(e)
cfg['semantic_topn_chunk'] = int(c)
cfg['graph_reserve_topn'] = int(r)
...
for ds in ('hotpotqa', '2wikimultihopqa'):
    ...
    proposal_ref = ((refs.get('current_proposal_round_best', {}) or {}).get(ds, {}) or {})
    if proposal_ref:
        fake_p = {
            'supporting_fact_recall': 0.0,
            'supporting_fact_recall_at_1': _f(proposal_ref.get('r1', 0.0)),
            'supporting_fact_recall_at_5': _f(proposal_ref.get('r5', 0.0)),
            'supporting_fact_recall_at_20': _f(proposal_ref.get('r20', 0.0)),
            'rendered_supporting_fact_recall': 0.0,
            'em': _f(proposal_ref.get('em', 0.0)),
            'f1': _f(proposal_ref.get('f1', 0.0)),
            'retrieval_latency_ms': 0.0,
            'total_latency_ms': 0.0,
        }
        variant = f"current_proposal_round_best({proposal_ref.get('variant','')})"
        rows.append(_row('reference', variant, ds, str(proposal_ref.get('summary_path', '')), fake_p))
...
lines = [
    '# Overnight Regime Isolation + Calibration Metrics (RAG full)',
    '',
...
read -r OLD_MAX_LEN OLD_MAX_CHARS NEW_MAX_LEN NEW_MAX_CHARS <<EOF
$("${PYTHON_BIN}" - "${REF_BUNDLE_JSON}" <<'PY'
import json,sys
obj=json.loads(open(sys.argv[1], 'r', encoding='utf-8').read())
emb=obj.get('embedding_regimes', {}) or {}
old=emb.get('old', {}) or {}
new=emb.get('new', {}) or {}
print(
    int(old.get('embedding_max_length', 192)),
    int(old.get('embedding_text_max_chars', 600)),
    int(new.get('embedding_max_length', 320)),
    int(new.get('embedding_text_max_chars', 900)),
)
PY
)
EOF
log_msg "[INFO] embedding_regime_old=max_length:${OLD_MAX_LEN},max_chars:${OLD_MAX_CHARS}"
log_msg "[INFO] embedding_regime_new=max_length:${NEW_MAX_LEN},max_chars:${NEW_MAX_CHARS}"
...
log_msg "[Set R] R1 hotpot_baseline_old_embed"
run_variant_for_dataset "set_r" "hotpot_baseline_old_embed" "hotpotqa" "hotpot_f2" "old" "off" "hotpot_fixed" 4 1 "${LIMIT_RAG}"

log_msg "[Set R] R2 hotpot_baseline_new_embed"
run_variant_for_dataset "set_r" "hotpot_baseline_new_embed" "hotpotqa" "hotpot_f2" "new" "off" "hotpot_fixed" 4 1 "${LIMIT_RAG}"

log_msg "[Set R] R3 wiki_baseline_old_embed"
run_variant_for_dataset "set_r" "wiki_baseline_old_embed" "2wikimultihopqa" "${WIKI_CORE_PROFILE}" "old" "off" "wiki_fixed" 3 1 "${LIMIT_RAG}"

log_msg "[Set R] R4 wiki_baseline_new_embed"
run_variant_for_dataset "set_r" "wiki_baseline_new_embed" "2wikimultihopqa" "${WIKI_CORE_PROFILE}" "new" "off" "wiki_fixed" 3 1 "${LIMIT_RAG}"

log_msg "[Set C] calibration embedding_regime=${CALIBRATION_EMBEDDING_REGIME}"

log_msg "[Set C] C1 hotpot_entity_up_chunk_down"
run_variant_for_dataset "set_c" "hotpot_entity_up_chunk_down" "hotpotqa" "hotpot_f2" "${CALIBRATION_EMBEDDING_REGIME}" "entity_up_chunk_down" "hotpot_fixed" 4 1 "${LIMIT_RAG}"

log_msg "[Set C] C2 wiki_entity_up_chunk_down"
run_variant_for_dataset "set_c" "wiki_entity_up_chunk_down" "2wikimultihopqa" "${WIKI_CORE_PROFILE}" "${CALIBRATION_EMBEDDING_REGIME}" "entity_up_chunk_down" "wiki_fixed" 3 1 "${LIMIT_RAG}"

log_msg "[Set C] C3 hotpot_entity_down_chunk_up"
run_variant_for_dataset "set_c" "hotpot_entity_down_chunk_up" "hotpotqa" "hotpot_f2" "${CALIBRATION_EMBEDDING_REGIME}" "entity_down_chunk_up" "hotpot_fixed" 4 1 "${LIMIT_RAG}"

log_msg "[Set C] C4 wiki_entity_down_chunk_up"
run_variant_for_dataset "set_c" "wiki_entity_down_chunk_up" "2wikimultihopqa" "${WIKI_CORE_PROFILE}" "${CALIBRATION_EMBEDDING_REGIME}" "entity_down_chunk_up" "wiki_fixed" 3 1 "${LIMIT_RAG}"

log_msg "[Set C] C5 hotpot_reserve_down"
run_variant_for_dataset "set_c" "hotpot_reserve_down" "hotpotqa" "hotpot_f2" "${CALIBRATION_EMBEDDING_REGIME}" "reserve_down" "hotpot_fixed" 4 1 "${LIMIT_RAG}"

log_msg "[Set C] C6 wiki_reserve_down"
run_variant_for_dataset "set_c" "wiki_reserve_down" "2wikimultihopqa" "${WIKI_CORE_PROFILE}" "${CALIBRATION_EMBEDDING_REGIME}" "reserve_down" "wiki_fixed" 3 1 "${LIMIT_RAG}"
