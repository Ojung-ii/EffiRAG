...
if [[ -n "${RESUME_ROUND_ROOT:-}" ]]; then
  ...
  ROUND_ROOT="outputs/overnight_path_complete_corridor/${RUN_STAMP}"
fi

LOG_ROOT="logs/overnight_path_complete_corridor/$(basename "${ROUND_ROOT}")"
...
maybe_snapshot_git_state() {
  ...
  if [[ "${ENABLE_GIT_SNAPSHOT:-false}" != "true" ]]; then
    ...
    log_msg "[INFO]   git commit -m 'chore: snapshot before path-complete corridor shaping'"
    return 0
  fi

  git add -A
  if ! git diff --cached --quiet; then
    git commit -m "chore: snapshot before path-complete corridor shaping"
  else
...
lines = [
    '# Overnight Path-Complete Corridor Metrics (RAG full)',
    '',
    '| stage | variant | dataset | R@5 | EM | F1 | path_complete_rate | bridge_answer_pair_retention | incomplete_path_rate | chain_compactness | path_preserve_activation | conversion_after_path_preserve | bridge_purity | answer_side_density | support_set_compactness | err_bridge_missing | err_answer_missing | err_bridge_noisy | err_gen_fail | oracle_gap | gap_to_hippo_r5 | gap_to_hippo_f1 |',
    '|---|---|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|',
]
for r in rows:
    lines.append(
        f"| {r['stage']} | {r['variant']} | {r['dataset']} | {r['Recall@5']:.4f} | {r['EM']:.4f} | {r['F1']:.4f} | {r['path_complete_rate']:.4f} | {r['bridge_answer_pair_retention']:.4f} | {r['incomplete_path_rate']:.4f} | {r['chain_compactness']:.4f} | {r['path_preserve_activation_rate']:.4f} | {r['conversion_after_path_preserve']:.4f} | {r['bridge_purity']:.4f} | {r['answer_side_density']:.4f} | {r['support_set_compactness']:.4f} | {r['error_bucket_bridge_missing']:.4f} | {r['error_bucket_answer_side_missing']:.4f} | {r['error_bucket_bridge_present_noisy']:.4f} | {r['error_bucket_answer_present_generation_fail']:.4f} | {r['oracle_gap']:+.4f} | {r['gap_to_hippo_r5']:+.4f} | {r['gap_to_hippo_f1']:+.4f} |"
    )
Path(out_md).write_text('\n'.join(lines) + '\n', encoding='utf-8')
PY
}
...
payload = {
    ...
    "fixed_by": "manual_freeze_for_path_complete_corridor_round",
...
log_msg "[Stage P3] r2_plus_path_preserve_compact"
run_variant_for_dataset "p3" "r2_plus_path_preserve_compact" "hotpotqa" "hotpot_f2" "${UNIVERSAL_REGIME}" "r2_plus_path_preserve_compact" "inherit" 0 0 "${LIMIT_RAG}"
run_variant_for_dataset "p3" "r2_plus_path_preserve_compact" "2wikimultihopqa" "${WIKI_CORE_PROFILE}" "${UNIVERSAL_REGIME}" "r2_plus_path_preserve_compact" "inherit" 0 0 "${LIMIT_RAG}"
