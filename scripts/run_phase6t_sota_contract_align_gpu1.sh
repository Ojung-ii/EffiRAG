#!/usr/bin/env bash
set -euo pipefail

cd /home/ojungii/EffiRAG

OUT_ROOT="${OUT_ROOT:-outputs/phase6t_sota_contract_align_n200}"
SAMPLE_SIZE="${SAMPLE_SIZE:-200}"
PROCESS_COUNT="${PROCESS_COUNT:-3}"
RUN_LIGHT="${RUN_LIGHT:-0}"
GPU="${GPU:-1}"
STRICT_VALIDATE="${STRICT_VALIDATE:-1}"
PYTHON="${PYTHON:-/home/ojungii/miniconda3/envs/effirag/bin/python}"

if [[ "${PROCESS_COUNT}" != "2" && "${PROCESS_COUNT}" != "3" ]]; then
  echo "[ERROR] PROCESS_COUNT must be 2 or 3, got: ${PROCESS_COUNT}" >&2
  exit 1
fi

if [[ -d "${OUT_ROOT}/qa_runs" ]] && [[ "${ALLOW_REUSE_OUT_ROOT:-0}" != "1" ]]; then
  echo "[ERROR] OUT_ROOT already contains qa_runs: ${OUT_ROOT}" >&2
  echo "Use a fresh OUT_ROOT or set ALLOW_REUSE_OUT_ROOT=1 explicitly." >&2
  exit 1
fi

mkdir -p "${OUT_ROOT}/logs"

echo "=== Phase-6T SOTA-contract alignment (GPU1-only) ==="
echo "OUT_ROOT=${OUT_ROOT}"
echo "SAMPLE_SIZE=${SAMPLE_SIZE}"
echo "PROCESS_COUNT=${PROCESS_COUNT}"
echo "RUN_LIGHT=${RUN_LIGHT}"
echo "GPU=${GPU}"

echo "=== Phase-6T methodology audit gate ==="
PYTHONPATH=. "${PYTHON}" scripts/audit_phase6t_sota_contract.py \
  --profile unified_acr_rcedr_v12_sota_contract_unified \
  --datasets hotpotqa 2wikimultihopqa \
  --check-configs configs/main_config/copy_span_instruction_unified \
  --out-root "${OUT_ROOT}" \
  2>&1 | tee "${OUT_ROOT}/logs/audit_phase6t_sota_contract.log"

SCHEDULE_JSONL="${OUT_ROOT}/_phase6t_scheduled_runs.jsonl"
: > "${SCHEDULE_JSONL}"

register_run () {
  local kind="$1"
  local profile="$2"
  local dataset="$3"
  local run_name="$4"
  local group="$5"
  local role="$6"
  printf '{"run_name":"%s","dataset":"%s","profile":"%s","gpu":"%s","process_group":"%s","role":"%s","kind":"%s","scheduled":true}\n' \
    "${run_name}" "${dataset}" "${profile}" "${GPU}" "${group}" "${role}" "${kind}" >> "${SCHEDULE_JSONL}"
}

run_unified_qa_one_dataset () {
  local profile="$1"
  local dataset="$2"
  local run_name="$3"
  echo "=== [GPU ${GPU}] profile=${profile} dataset=${dataset} run=${run_name} ==="
  CUDA_VISIBLE_DEVICES="${GPU}" PYTHONPATH=. "${PYTHON}" \
    scripts/run_unified_config_4ds.py \
    --profile "${profile}" \
    --datasets "${dataset}" \
    --sample-size "${SAMPLE_SIZE}" \
    --output-root "${OUT_ROOT}/qa_runs/${run_name}" \
    2>&1 | tee "${OUT_ROOT}/logs/qa_${run_name}.log"
}

run_legacy_qa_one_dataset () {
  local dataset="$1"
  local run_name="$2"
  echo "=== [GPU ${GPU}] legacy_sota dataset=${dataset} run=${run_name} ==="
  CUDA_VISIBLE_DEVICES="${GPU}" PYTHONPATH=. "${PYTHON}" \
    scripts/run_sota_config_4ds.py \
    --mode locked_precomputed \
    --datasets "${dataset}" \
    --sample-size "${SAMPLE_SIZE}" \
    --output-root "${OUT_ROOT}/qa_runs/${run_name}" \
    2>&1 | tee "${OUT_ROOT}/logs/qa_${run_name}.log"
}

run_task () {
  local spec="$1"
  IFS='|' read -r kind profile dataset run_name group role <<< "${spec}"
  rm -rf "${OUT_ROOT}/qa_runs/${run_name}"
  mkdir -p "${OUT_ROOT}/qa_runs/${run_name}"
  if [[ "${kind}" == "legacy" ]]; then
    run_legacy_qa_one_dataset "${dataset}" "${run_name}"
  else
    run_unified_qa_one_dataset "${profile}" "${dataset}" "${run_name}"
  fi
}

TASKS_A=()
TASKS_B=()
TASKS_C=()

if [[ "${PROCESS_COUNT}" == "3" ]]; then
  TASKS_A=(
    "legacy|legacy_sota|hotpotqa|legacy_sota_hotpotqa|A|reference"
    "unified|unified_acr_rcedr_v12_sota_contract_unified|2wikimultihopqa|v12_sota_contract_unified_2wiki|A|contract_alignment"
    "unified|unified_large|hotpotqa|unified_large_hotpotqa|A|baseline"
  )
  TASKS_B=(
    "legacy|legacy_sota|2wikimultihopqa|legacy_sota_2wiki|B|reference"
    "unified|unified_acr_rcedr_v12_sota_contract_unified|hotpotqa|v12_sota_contract_unified_hotpotqa|B|contract_alignment"
    "unified|unified_large|2wikimultihopqa|unified_large_2wiki|B|baseline"
  )
  TASKS_C=(
    "unified|unified_acr_rcedr_v12|hotpotqa|v12_hotpotqa|C|current_main"
    "unified|unified_acr_rcedr_v12|2wikimultihopqa|v12_2wiki|C|current_main"
  )
else
  TASKS_A=(
    "legacy|legacy_sota|hotpotqa|legacy_sota_hotpotqa|A|reference"
    "unified|unified_large|hotpotqa|unified_large_hotpotqa|A|baseline"
    "unified|unified_acr_rcedr_v12|hotpotqa|v12_hotpotqa|A|current_main"
    "unified|unified_acr_rcedr_v12_sota_contract_unified|2wikimultihopqa|v12_sota_contract_unified_2wiki|A|contract_alignment"
  )
  TASKS_B=(
    "legacy|legacy_sota|2wikimultihopqa|legacy_sota_2wiki|B|reference"
    "unified|unified_large|2wikimultihopqa|unified_large_2wiki|B|baseline"
    "unified|unified_acr_rcedr_v12|2wikimultihopqa|v12_2wiki|B|current_main"
    "unified|unified_acr_rcedr_v12_sota_contract_unified|hotpotqa|v12_sota_contract_unified_hotpotqa|B|contract_alignment"
  )
fi

if [[ "${RUN_LIGHT}" == "1" ]]; then
  TASKS_A+=("unified|unified_acr_rcedr_v12_sota_contract_light|hotpotqa|v12_sota_contract_light_hotpotqa|A|latency_ablation")
  TASKS_B+=("unified|unified_acr_rcedr_v12_sota_contract_light|2wikimultihopqa|v12_sota_contract_light_2wiki|B|latency_ablation")
fi

ALL_TASKS=("${TASKS_A[@]}" "${TASKS_B[@]}" "${TASKS_C[@]}")

declare -A RUN_NAME_COUNT
for spec in "${ALL_TASKS[@]}"; do
  IFS='|' read -r _kind _profile _dataset _run_name _group _role <<< "${spec}"
  RUN_NAME_COUNT["${_run_name}"]=$(( ${RUN_NAME_COUNT["${_run_name}"]:-0} + 1 ))
done
for run_name in "${!RUN_NAME_COUNT[@]}"; do
  if [[ "${RUN_NAME_COUNT[${run_name}]}" -gt 1 ]]; then
    echo "[ERROR] duplicate run_name detected: ${run_name}" >&2
    exit 1
  fi
done

for spec in "${ALL_TASKS[@]}"; do
  IFS='|' read -r kind profile dataset run_name group role <<< "${spec}"
  register_run "${kind}" "${profile}" "${dataset}" "${run_name}" "${group}" "${role}"
done

export OUT_ROOT SAMPLE_SIZE SCHEDULE_JSONL
PYTHONPATH=. "${PYTHON}" - <<'PY'
import json
import os
from datetime import datetime, timezone
from pathlib import Path

out_root = Path(os.environ["OUT_ROOT"]).resolve()
jsonl = Path(os.environ["SCHEDULE_JSONL"]).resolve()
runs = []
if jsonl.exists():
    for line in jsonl.read_text(encoding="utf-8").splitlines():
        line = line.strip()
        if not line:
            continue
        try:
            row = json.loads(line)
        except Exception:
            continue
        if isinstance(row, dict):
            runs.append(row)

manifest = {
    "phase": "phase6t_sota_contract_alignment",
    "sample_size": int(os.environ.get("SAMPLE_SIZE", "0")),
    "out_root": str(out_root),
    "created_at": datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ"),
    "runs": runs,
}

phase6t = out_root / "phase6t_run_manifest.json"
phase6s_alias = out_root / "phase6s_run_manifest.json"
phase6t.write_text(json.dumps(manifest, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
phase6s_alias.write_text(json.dumps(manifest, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
print(str(phase6t))
print(str(phase6s_alias))
PY

group_a () { local spec; for spec in "${TASKS_A[@]}"; do run_task "${spec}"; done; }
group_b () { local spec; for spec in "${TASKS_B[@]}"; do run_task "${spec}"; done; }
group_c () { local spec; for spec in "${TASKS_C[@]}"; do run_task "${spec}"; done; }

group_a > "${OUT_ROOT}/logs/group_A_gpu1.log" 2>&1 &
PID_A=$!
group_b > "${OUT_ROOT}/logs/group_B_gpu1.log" 2>&1 &
PID_B=$!

if [[ "${PROCESS_COUNT}" == "3" ]]; then
  group_c > "${OUT_ROOT}/logs/group_C_gpu1.log" 2>&1 &
  PID_C=$!
fi

FAIL=0
if ! wait "${PID_A}"; then FAIL=1; fi
if ! wait "${PID_B}"; then FAIL=1; fi
if [[ "${PROCESS_COUNT}" == "3" ]]; then
  if ! wait "${PID_C}"; then FAIL=1; fi
fi
if [[ "${FAIL}" != "0" ]]; then
  echo "[ERROR] One or more workers failed. Check ${OUT_ROOT}/logs/group_*.log" >&2
  exit 1
fi

echo "=== summarize Phase-6T ==="
PYTHONPATH=. "${PYTHON}" scripts/summarize_phase6t_sota_contract_alignment.py \
  --out-root "${OUT_ROOT}" \
  2>&1 | tee "${OUT_ROOT}/logs/summarize_phase6t.log"

echo "=== artifact consistency validation ==="
VALIDATE_ARGS=()
if [[ "${STRICT_VALIDATE}" == "1" ]]; then
  VALIDATE_ARGS+=(--strict-clean)
fi
PYTHONPATH=. "${PYTHON}" scripts/validate_phase6s_artifacts.py \
  --out-root "${OUT_ROOT}" \
  "${VALIDATE_ARGS[@]}" \
  --json "${OUT_ROOT}/phase6t_artifact_validation.json" \
  2>&1 | tee "${OUT_ROOT}/logs/validate_phase6t_artifacts.log"

echo "[Phase-6T] scheduled runs:"
PYTHONPATH=. "${PYTHON}" - <<'PY'
import json
import os

path = os.path.join(os.environ["OUT_ROOT"], "phase6t_run_manifest.json")
if os.path.exists(path):
    print(len(json.load(open(path)).get("runs", [])))
else:
    print("missing phase6t_run_manifest.json")
PY

echo "[Phase-6T] rag_summary count:"
find "${OUT_ROOT}" -name rag_summary.json | wc -l

echo "[Phase-6T] rag_query_results count:"
find "${OUT_ROOT}" -name rag_query_results.jsonl | wc -l

echo "=== done ==="
echo "Audit: ${OUT_ROOT}/PHASE6T_SOTA_CONTRACT_AUDIT.md"
echo "Summary: ${OUT_ROOT}/PHASE6T_SOTA_CONTRACT_ALIGNMENT_SUMMARY.md"
echo "Summary JSON: ${OUT_ROOT}/phase6t_sota_contract_alignment_summary.json"
