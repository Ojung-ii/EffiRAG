...
def _build_query_row(
    *,
    ...
    query_row: Dict[str, Any] = {
        ...
        "adaptive_support_span_enabled": bool(rendered_meta.get("adaptive_support_span_enabled", False)),
        ...
        "length_penalty_avg": float(_safe_float(rendered_meta.get("length_penalty_avg", 0.0), 0.0)),
...
def main() -> None:
    ...
    adaptive_support_span_rows: List[Dict[str, Any]] = []
    for row in query_rows:
            ...
            support_span_contract_rows.append(
                {
                    ...
                    "length_penalty_avg": float(_safe_float(row.get("length_penalty_avg", 0.0), 0.0)),
                }
            )
        if bool(row.get("adaptive_support_span_enabled", False)):
            adaptive_support_span_rows.append(
                {
                    "dataset": str(row.get("dataset", "")),
                    "qid": str(row.get("qid", "")),
                    "profile": str(row.get("profile", "")),
    ...
    if adaptive_support_span_rows:
        write_jsonl(output_dir / "query_level_adaptive_support_span.jsonl", adaptive_support_span_rows)
    write_csv(
