...
GPU="${GPU:-1}"
OUT_ROOT="${OUT_ROOT:-outputs/phase6k_sentence_contract}"
DATASETS="${DATASETS:-hotpotqa 2wikimultihopqa}"
SAMPLE_SIZE="${SAMPLE_SIZE:-100}"

mkdir -p "${OUT_ROOT}/logs"

echo "=== Phase-6K Sentence Contract retrieval-only run ==="
echo "GPU=${GPU}"
...
run_unified unified_gl_rcedr_v1_sentence_contract_metadata_on unified_gl_rcedr_v1_sentence_contract_metadata_on
run_unified unified_gl_rcedr_v1_sentence_contract_span40 unified_gl_rcedr_v1_sentence_contract_span40
...
"${PYTHON}" scripts/audit_evidence_flow.py \
  ...
  --roots \
    ...
    unified_gl_rcedr_v1_sentence_contract_metadata_on="${OUT_ROOT}/runs/unified_gl_rcedr_v1_sentence_contract_metadata_on" \
    unified_gl_rcedr_v1_sentence_contract_span40="${OUT_ROOT}/runs/unified_gl_rcedr_v1_sentence_contract_span40" \
