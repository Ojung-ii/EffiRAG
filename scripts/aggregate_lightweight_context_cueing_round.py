...
from effirag.eval_metrics import (
    MetricRegistry,
    build_lightweight_context_cueing_metric_registry,
...
def _build_per_query_metrics(
    *,
    ...
    for i, raw in enumerate(list(rows or [])):
        ...
        role_prefix_activation_q = _safe_float(rendered_meta.get("role_prefix_activation_rate", 0.0), 0.0)
        source_role_hint_activation_q = _safe_float(rendered_meta.get("source_role_hint_activation_rate", 0.0), 0.0)
        role_unknown_rate_q = _safe_float(rendered_meta.get("role_unknown_rate", 0.0), 0.0)
        if role_prefix_activation_q > 0.0:
            role_unknown_rate_q = _safe_float(rendered_meta.get("role_prefix_unknown_rate", role_unknown_rate_q), role_unknown_rate_q)
        elif source_role_hint_activation_q > 0.0:
            role_unknown_rate_q = _safe_float(rendered_meta.get("source_role_hint_unknown_rate", role_unknown_rate_q), role_unknown_rate_q)

        cueing_variant = _strip(rendered_meta.get("render_variant") or variant)
        ...
        out.append(
            {
                ...
                "cueing_variant": str(cueing_variant or variant),
                "role_prefix_activation_q": float(max(0.0, min(1.0, role_prefix_activation_q))),
                "role_unknown_rate_q": float(max(0.0, min(1.0, role_unknown_rate_q))),
                "source_role_hint_activation_q": float(max(0.0, min(1.0, source_role_hint_activation_q))),
                "duplicated_title_reduction_rate_q": float(max(0.0, _safe_float(rendered_meta.get("duplicated_title_reduction_rate", 0.0), 0.0))),
                "unique_title_count_q": float(max(0.0, _safe_float(rendered_meta.get("unique_title_count", 0.0), 0.0))),
                "title_repetition_reduction_q": float(max(0.0, _safe_float(rendered_meta.get("title_repetition_reduction", 0.0), 0.0))),
                "separator_activation_q": float(max(0.0, min(1.0, _safe_float(rendered_meta.get("separator_activation_rate", 0.0), 0.0)))),
                "reorder_activation_q": float(max(0.0, min(1.0, _safe_float(rendered_meta.get("reorder_activation_rate", 0.0), 0.0)))),
                "avg_rank_shift_q": float(max(0.0, _safe_float(rendered_meta.get("avg_rank_shift", 0.0), 0.0))),
                "max_rank_shift_q": float(max(0.0, _safe_float(rendered_meta.get("max_rank_shift", 0.0), 0.0))),
                ...
                "corridor_or_path_info_present": 1.0 if bool(corridor_or_path_ids) else 0.0,
                ...
                "missing_rendered_context": 1.0 if not bool(rendered_context) else 0.0,
...
def _aggregate_variant(
    *,
    ...
    retrieval_source_breakdown = dict(summary.get("retrieval_source_breakdown", {}) or {})
    out = {
        "dataset": str(dataset),
        "variant": str(variant),
        "query_count": int(n),
        ...
        "cueing_variant": _mode_text([str(r.get("cueing_variant", "")) for r in rows], default=str(variant)),
        ...
        "role_prefix_activation_rate": _rate("role_prefix_activation_q"),
        "role_unknown_rate": float(_mean([_safe_float(r.get("role_unknown_rate_q", 0.0), 0.0) for r in rows])),
        "source_role_hint_activation_rate": _rate("source_role_hint_activation_q"),
        "duplicated_title_reduction_rate": _rate("duplicated_title_reduction_rate_q"),
        "unique_title_count": float(_mean([_safe_float(r.get("unique_title_count_q", 0.0), 0.0) for r in rows])),
        "title_repetition_reduction": _rate("title_repetition_reduction_q"),
        "separator_activation_rate": _rate("separator_activation_q"),
        "reorder_activation_rate": _rate("reorder_activation_q"),
        "avg_rank_shift": float(_mean([_safe_float(r.get("avg_rank_shift_q", 0.0), 0.0) for r in rows])),
        "max_rank_shift": float(_mean([_safe_float(r.get("max_rank_shift_q", 0.0), 0.0) for r in rows])),
        ...
        "missing_rendered_context_rate": _rate("missing_rendered_context"),
        ...
        "summary_precomputed_retrieval_used": 1.0 if _safe_bool(summary.get("precomputed_retrieval_used", False), False) else 0.0,
        "summary_precomputed_retrieval_strict": 1.0 if _safe_bool(summary.get("precomputed_retrieval_strict", False), False) else 0.0,
        "summary_retrieval_source_precomputed": _safe_float(retrieval_source_breakdown.get("precomputed", 0.0), 0.0),
        "summary_retrieval_source_on_the_fly": _safe_float(retrieval_source_breakdown.get("on_the_fly", 0.0), 0.0),
    }
    return out
        ...
        out.append(
            {
                ...
                "recall_at_1_delta": float(d1),
                "recall_at_5_delta": float(d5),
                "recall_at_10_delta": float(d10),
                "retrieval_invariance_pass": "pass" if (abs(d1) <= 1.0e-9 and abs(d5) <= 1.0e-9 and abs(d10) <= 1.0e-9) else "fail",
...
def _build_retrieval_invariance_rows(metrics_rows: Sequence[Mapping[str, Any]], baseline_variant: str) -> List[Dict[str, Any]]:
    items = [dict(x or {}) for x in list(metrics_rows or [])]
    idx = {(str(r.get("dataset", "")), str(r.get("variant", ""))): r for r in items}
    out: List[Dict[str, Any]] = []
    for row in items:
        ds = str(row.get("dataset", ""))
        ...
        pass_flag = "pass" if (abs(d1) <= 1.0e-9 and abs(d5) <= 1.0e-9 and abs(d10) <= 1.0e-9) else "fail"
        rr = dict(row)
        rr["recall_at_1_delta"] = float(d1)
        rr["recall_at_5_delta"] = float(d5)
        rr["recall_at_10_delta"] = float(d10)
        rr["retrieval_invariance_pass"] = pass_flag
        out.append(rr)

    out.sort(key=lambda x: (str(x.get("dataset", "")), str(x.get("variant", ""))))
    return out
...
def main() -> None:
    ...
    out_metrics_json = round_root / "lightweight_context_cueing_metrics.json"
    out_metrics_md = round_root / "lightweight_context_cueing_metrics.md"
    out_delta_md = round_root / "lightweight_context_cueing_delta_table.md"
    ...
    out_cueing_md = round_root / "cueing_variant_table.md"
    ...
    registry: MetricRegistry = build_lightweight_context_cueing_metric_registry()
    ...
    baseline_index = {(str(r.get("dataset", "")), str(r.get("variant", ""))): r for r in metrics_rows}
    ...
    for row in metrics_rows:
        ...
        if base is None:
            ...
            row["retrieval_invariance_pass"] = "pass"
            continue
        ...
        row["recall_at_1_delta"] = _safe_float(row.get("recall_at_1", 0.0), 0.0) - _safe_float(base.get("recall_at_1", 0.0), 0.0)
        row["recall_at_5_delta"] = _safe_float(row.get("recall_at_5", 0.0), 0.0) - _safe_float(base.get("recall_at_5", 0.0), 0.0)
        row["recall_at_10_delta"] = _safe_float(row.get("recall_at_10", 0.0), 0.0) - _safe_float(base.get("recall_at_10", 0.0), 0.0)
        row["retrieval_invariance_pass"] = (
            "pass"
            if (
                abs(_safe_float(row.get("recall_at_1_delta", 0.0), 0.0)) <= 1.0e-9
                and abs(_safe_float(row.get("recall_at_5_delta", 0.0), 0.0)) <= 1.0e-9
                and abs(_safe_float(row.get("recall_at_10_delta", 0.0), 0.0)) <= 1.0e-9
            )
            else "fail"
        )

    retrieval_invariance_rows = _build_retrieval_invariance_rows(metrics_rows, baseline_variant="champion_reconfirm")
    retrieval_invariance_violations = [
        ...
        for r in retrieval_invariance_rows
        if _strip(r.get("retrieval_invariance_pass")) == "fail"
    ]
    ...
    retrieval_fixed_violations = [
        {
            "dataset": str(r.get("dataset", "")),
            "variant": str(r.get("variant", "")),
            "retrieval_source_on_the_fly_rate": _safe_float(r.get("retrieval_source_on_the_fly_rate", 0.0), 0.0),
            "summary_retrieval_source_on_the_fly": _safe_float(r.get("summary_retrieval_source_on_the_fly", 0.0), 0.0),
        }
        for r in metrics_rows
        if (
            _safe_float(r.get("retrieval_source_on_the_fly_rate", 0.0), 0.0) > 1.0e-12
            or _safe_float(r.get("summary_retrieval_source_on_the_fly", 0.0), 0.0) > 1.0e-12
        )
    ]
    ...
    write_markdown(
        str(out_metrics_md),
        markdown_document_from_group_tables(group_tables, title="Lightweight Context Cueing Round"),
    )
    ...
    cueing_md = build_flat_markdown_table(
        records=metrics_rows,
        headers=[
            "dataset",
            "variant",
            "cueing_variant",
            "role_prefix_activation_rate",
            "role_unknown_rate",
            "source_role_hint_activation_rate",
            "duplicated_title_reduction_rate",
            "unique_title_count",
            "title_repetition_reduction",
            "separator_activation_rate",
            "reorder_activation_rate",
            "avg_rank_shift",
            "max_rank_shift",
        ],
        registry=registry,
    )
    write_markdown(str(out_cueing_md), cueing_md + "\n")
    ...
    retrieval_inv_md = build_flat_markdown_table(
    ...
    write_markdown(str(out_retrieval_inv_md), retrieval_inv_md + "\n")
    ...
    notes = metric_definition_notes(
        ...
        tokenizer_name=token_counter.tokenizer_name,
        optional_notes=[
            "Lightweight context cueing round is retrieval-fixed: all variants load champion precomputed retrieval artifacts.",
            ...
            "no prediction rewrite constraint is checked by qa_utilization_changed_rate and no_prediction_rewrite_violation_rate.",
        ],
    )
