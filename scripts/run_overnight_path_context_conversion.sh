...
maybe_snapshot_git_state() {
  ...
  if [[ "${ENABLE_GIT_SNAPSHOT:-false}" != "true" ]]; then
    ...
    log_msg "[INFO]   git commit -m 'chore: snapshot before path-to-context conversion refinement'"
    return 0
  fi

  git add -A
  if ! git diff --cached --quiet; then
    git commit -m "chore: snapshot before path-to-context conversion refinement"
  else
...
if tag in {'r2_bridge_only', 'bridge_candidate_induction'}:
    ...
    'r2_plus_path_preserve_path_ordered',
    'r2_plus_path_preserve_path_ordered_dedup',
    'r2_plus_path_preserve_path_ordered_compactlite',
}:
    cfg['retrieval_objective_mode'] = 'r2_plus_path_preserve'
    if tag == 'r2_plus_path_preserve_reconfirm':
        pass
    elif tag == 'r2_plus_path_preserve_path_ordered':
        cfg['render_mode'] = 'path_bundle'
        cfg['order_strategy'] = 'path_bundle_ordered'
        cfg['chunk_grounding_enabled'] = False
    elif tag == 'r2_plus_path_preserve_path_ordered_dedup':
        cfg['render_mode'] = 'path_bundle'
        cfg['order_strategy'] = 'path_bundle_ordered+path_bundle_dedup'
        cfg['chunk_grounding_enabled'] = False
    elif tag == 'r2_plus_path_preserve_path_ordered_compactlite':
        cfg['render_mode'] = 'path_bundle'
        cfg['order_strategy'] = 'path_bundle_ordered+path_bundle_dedup+path_bundle_compactlite'
        cfg['chunk_grounding_enabled'] = False
        cfg['max_corridors_in_context'] = min(int(cfg.get('max_corridors_in_context', 3) or 3), 2)
        cfg['max_main_sentences_per_corridor'] = min(int(cfg.get('max_main_sentences_per_corridor', 3) or 3), 2)
        cfg['max_support_per_corridor'] = min(int(cfg.get('max_support_per_corridor', 2) or 2), 2)
        cfg['top_corridors'] = min(int(cfg.get('top_corridors', 3) or 3), 2)
        cfg['max_sentences'] = min(int(cfg.get('max_sentences', 10) or 10), int(cfg.get('max_context_sentences', 8) or 8))
...
lines = [
    '# Overnight Path-to-Context Conversion Metrics (RAG full)',
    '',
...
lines += [
    '',
    '## Path-to-Context Diagnostic Table',
    '',
    '| stage | variant | dataset | path_bundle_count | avg_chunks_per_bundle | bridge_answer_adjacency_rate | first_complete_path_rank | bundle_dedup_ratio | answer_path_coverage | answer_present_but_generation_fail | conversion_after_path_bundle |',
    '|---|---|---|---:|---:|---:|---:|---:|---:|---:|---:|',
]
for r in rows:
    if str(r.get('stage')) == 'reference':
        continue
    lines.append(
        f"| {r['stage']} | {r['variant']} | {r['dataset']} | {r['path_bundle_count']:.4f} | {r['avg_chunks_per_bundle']:.4f} | {r['bridge_answer_adjacency_rate']:.4f} | {r['first_complete_path_rank']:.4f} | {r['bundle_dedup_ratio']:.4f} | {r['answer_path_coverage']:.4f} | {r['answer_present_but_generation_fail']:.4f} | {r['conversion_after_path_bundle']:.4f} |"
    )
...
payload = {
    ...
    "fixed_by": "manual_freeze_for_path_context_conversion_round",
