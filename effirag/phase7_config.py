...
class Phase7Config:
    ...
    source_balanced_proposal_enabled: bool = False
    source_balanced_candidate_top_m: int = 128
    source_balanced_fill_remaining: bool = True
    source_quota_semantic: int = 48
    source_quota_entity_title: int = 32
    source_quota_graph_flow: int = 32
    source_quota_anchor_neighborhood: int = 16
    chain_unit_enabled: bool = False
    chain_unit_max_pair_units: int = 256
    chain_unit_use_same_title: bool = True
    chain_unit_use_explicit_transition: bool = True
    chain_unit_use_same_carrier: bool = True
    chain_unit_use_shared_entity: bool = False
    chain_unit_max_unit_size: int = 2
...
def phase7_config_from_cfg(cfg: Any) -> Phase7Config:
    ...
    return Phase7Config(
        ...
        source_balanced_proposal_enabled=_to_bool(
            getattr(cfg, "phase7_source_balanced_proposal_enabled", False),
            False,
        ),
        source_balanced_candidate_top_m=max(
            1, _to_int(getattr(cfg, "phase7_source_balanced_candidate_top_m", 128), 128)
        ),
        source_balanced_fill_remaining=_to_bool(
            getattr(cfg, "phase7_source_balanced_fill_remaining", True),
            True,
        ),
        source_quota_semantic=max(
            0, _to_int(getattr(cfg, "phase7_source_quota_semantic", 48), 48)
        ),
        source_quota_entity_title=max(
            0, _to_int(getattr(cfg, "phase7_source_quota_entity_title", 32), 32)
        ),
        source_quota_graph_flow=max(
            0, _to_int(getattr(cfg, "phase7_source_quota_graph_flow", 32), 32)
        ),
        source_quota_anchor_neighborhood=max(
            0, _to_int(getattr(cfg, "phase7_source_quota_anchor_neighborhood", 16), 16)
        ),
        chain_unit_enabled=_to_bool(
            getattr(cfg, "phase7_chain_unit_enabled", False),
            False,
        ),
        chain_unit_max_pair_units=max(
            0, _to_int(getattr(cfg, "phase7_chain_unit_max_pair_units", 256), 256)
        ),
        chain_unit_use_same_title=_to_bool(
            getattr(cfg, "phase7_chain_unit_use_same_title", True),
            True,
        ),
        chain_unit_use_explicit_transition=_to_bool(
            getattr(cfg, "phase7_chain_unit_use_explicit_transition", True),
            True,
        ),
        chain_unit_use_same_carrier=_to_bool(
            getattr(cfg, "phase7_chain_unit_use_same_carrier", True),
            True,
        ),
        chain_unit_use_shared_entity=_to_bool(
            getattr(cfg, "phase7_chain_unit_use_shared_entity", False),
            False,
        ),
        chain_unit_max_unit_size=max(
            1, _to_int(getattr(cfg, "phase7_chain_unit_max_unit_size", 2), 2)
        ),
    )
