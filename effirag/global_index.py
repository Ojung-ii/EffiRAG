...
def build_corpus_graph(
    ...
    build_sentence_layer = bool(text_unit == "sentence" or force_sentence_layer)
    doc_chunk_nodes = {}
...
def _normalize_build_config(
    ...
    return {
        ...
        "passage_chunk_size_sentences": max(1, int(passage_chunk_size_sentences)),
        "passage_chunk_stride_sentences": max(1, int(passage_chunk_stride_sentences)),
        "passage_chunk_min_sentences": max(1, int(passage_chunk_min_sentences)),
        "passage_chunk_max_chars": max(100, int(passage_chunk_max_chars)),
        "force_sentence_layer": bool(force_sentence_layer),
    }
...
def load_or_build_global_index(
    ...
    if prebuilt_path:
        ...
        graph, stats = build_corpus_graph(
            ...
            force_sentence_layer=bool(build_config.get("force_sentence_layer", False)),
            show_progress=show_progress,
        )
        stats = dict(stats or {})
