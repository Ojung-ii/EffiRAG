...
def _patch_transformers_mistral_position_embeddings_compat():
    try:
        import inspect
        ...
        return False, f"mistral_patch_import_failed: {exc}"

    try:
        attn_cls = getattr(mistral_mod, "MistralAttention", None)
        if attn_cls is None:
            return False, "mistral_attention_class_missing"
        current_forward = getattr(attn_cls, "forward", None)
        if current_forward is None:
            return False, "mistral_attention_forward_missing"
        if getattr(current_forward, "_effirag_nvembed_pos_patch", False):
            return False, "already_patched"

        sig = inspect.signature(current_forward)
        if "position_embeddings" not in sig.parameters:
            # Older Transformers versions do not require this kwarg.
            return False, "position_embeddings_not_required"
        ...
        def _patched_forward(
            ...
            **kwargs,
            ...
            # NV-Embed remote code may call Mistral layers without supplying
            # position_embeddings on newer Transformers. Fall back to identity
            # RoPE tensors to avoid runtime crashes.
            if position_embeddings is None:
                try:
                    if hidden_states is None or getattr(hidden_states, "ndim", 0) < 3:
                        raise RuntimeError("invalid_hidden_states_for_position_embedding_fallback")
                    batch_size = int(hidden_states.shape[0])
                    seq_len = int(hidden_states.shape[1])
                    head_dim = int(getattr(self, "head_dim", 0) or 0)
                    if head_dim <= 0:
                        head_dim = int(hidden_states.shape[-1])
                    cos = torch.ones(
                    ...
                    raise RuntimeError(f"position_embedding_fallback_failed: {exc}") from exc
        ...
        _patched_forward._effirag_nvembed_pos_patch = True  # type: ignore[attr-defined]
        setattr(attn_cls, "forward", _patched_forward)
        return True, "patched_mistral_attention_position_embeddings_fallback"
    except Exception as exc:
        return False, f"mistral_position_embeddings_patch_failed: {exc}"


def _patch_transformers_mistral_decoder_legacy_output_compat():
    try:
        import inspect
        from functools import wraps
        ...
        return False, f"mistral_decoder_patch_import_failed: {exc}"

    try:
        decoder_cls = getattr(mistral_mod, "MistralDecoderLayer", None)
        if decoder_cls is None:
            return False, "mistral_decoder_class_missing"
        current_forward = getattr(decoder_cls, "forward", None)
        if current_forward is None:
            return False, "mistral_decoder_forward_missing"
        if getattr(current_forward, "_effirag_nvembed_decoder_patch", False):
            return False, "already_patched"

        sig = inspect.signature(current_forward)
        if "use_cache" not in sig.parameters:
            return False, "decoder_use_cache_signature_missing"
        ...
        def _patched_decoder_forward(
            ...
            **kwargs,
            ...
            legacy_past = kwargs.pop("past_key_value", None)
            if past_key_values is None and legacy_past is not None:
            ...
            # Transformers>=4.57 returns only hidden_states tensor here.
            # NV-Embed remote code expects legacy tuple outputs.
            if isinstance(out, tuple):
                return out
            if (not bool(use_cache)) and (not output_attentions):
                return out
            if output_attentions and bool(use_cache):
                return (out, None, past_key_values)
            if output_attentions:
                return (out, None)
            return (out, past_key_values)

        _patched_decoder_forward._effirag_nvembed_decoder_patch = True  # type: ignore[attr-defined]
        setattr(decoder_cls, "forward", _patched_decoder_forward)
        return True, "patched_mistral_decoder_legacy_tuple_output"
    except Exception as exc:
        return False, f"mistral_decoder_patch_failed: {exc}"
...
def _patch_nvembed_runtime_compat(key):
    ...
    mistral_ok, mistral_msg = _patch_transformers_mistral_position_embeddings_compat()
    if mistral_ok or str(mistral_msg) in {"already_patched", "position_embeddings_not_required"}:
        notes.append(f"mistral_position_embeddings={mistral_msg}")
    else:
        notes.append(f"mistral_position_embeddings_patch_issue={mistral_msg}")

    decoder_ok, decoder_msg = _patch_transformers_mistral_decoder_legacy_output_compat()
    if decoder_ok or str(decoder_msg) in {"already_patched", "decoder_use_cache_signature_missing"}:
        notes.append(f"mistral_decoder_legacy_output={decoder_msg}")
    else:
        notes.append(f"mistral_decoder_legacy_output_patch_issue={decoder_msg}")
    return notes
...
def _init_model_with_fallbacks(key, torch, AutoModel):
    ...
    try:
        ...
        if fallback_dtype is not None:
            fallback_load_kwargs["torch_dtype"] = fallback_dtype
        model, notes = _load_model_with_optional_nvembed_patch(
            key=key,
            AutoModel=AutoModel,
            load_kwargs=fallback_load_kwargs,
        )
        errors.extend(notes)
        device = "cuda" if torch.cuda.is_available() else "cpu"
        if device == "cuda" and fallback_dtype is not None:
            try:
                model = model.to(device=device, dtype=fallback_dtype)
            except Exception:
    ...
    if backend == "hf_pool":
        try:
            ...
            try:
                ...
                _EMBEDDER_CACHE[key] = {
                    "ok": False,
                    "error": f"embedding_tokenizer_load_failed: local={exc_local} | remote={exc}",
                }
                return _EMBEDDER_CACHE[key]
...
def get_embedding_model_length_info(model_name):
    key = str(model_name or "").strip() or "sentence-transformers/all-MiniLM-L6-v2"
    info = {
        "model_name": key,
        "backend": "metadata_only",
        "error": "",
        "tokenizer_model_max_length": 0,
        "tokenizer_model_max_length_raw": 0,
    ...
    try:
        ...
        try:
            ...
            info["error"] = f"tokenizer_local_load_failed: {exc}"
        ...
        try:
            ...
            if not info["error"]:
                info["error"] = f"config_local_load_failed: {exc}"
    except Exception as exc:
        info["error"] = f"transformers_import_failed: {exc}"
        return info

    raw_model_max = 0
    try:
        raw_model_max = int(getattr(tokenizer, "model_max_length", 0) or 0) if tokenizer is not None else 0
    except Exception:
        raw_model_max = 0
    info["tokenizer_model_max_length_raw"] = int(raw_model_max)
    # Hugging Face may use a sentinel huge integer for "effectively unbounded".
    info["tokenizer_model_max_length"] = 0 if raw_model_max >= 10**8 else int(max(0, raw_model_max))

    max_seq = 0
    try:
        for key_name in ("max_seq_length", "max_position_embeddings", "n_positions", "max_seq_len"):
            val = int(getattr(config, key_name, 0) or 0)
            if val > 0:
                max_seq = val
                break
    except Exception:
        max_seq = 0
    info["model_max_seq_length"] = int(max(0, max_seq))
    return info
