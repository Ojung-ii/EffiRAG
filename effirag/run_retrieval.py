...
def execute_retrieval_experiment(cfg, show_progress: bool = True):
    ...
    summary["canonical_variant_name"] = str(getattr(cfg, "canonical_variant_name", ""))
    summary["run_light_rerank_enabled"] = bool(getattr(cfg, "run_light_rerank_enabled", False))
    summary["run_light_rerank_topk"] = int(getattr(cfg, "run_light_rerank_topk", 0))
    summary["run_light_rerank_weight_base"] = float(getattr(cfg, "run_light_rerank_weight_base", 0.0))
    summary["run_light_rerank_weight_bridge_completeness"] = float(
        getattr(cfg, "run_light_rerank_weight_bridge_completeness", 0.0)
    )
    summary["run_light_rerank_weight_anchor_coverage"] = float(
        getattr(cfg, "run_light_rerank_weight_anchor_coverage", 0.0)
    )
    summary["run_light_rerank_weight_grounding"] = float(getattr(cfg, "run_light_rerank_weight_grounding", 0.0))
    ...
    summary["max_anchors"] = int(cfg.max_anchors)
    summary["samples_per_anchor"] = int(cfg.samples_per_anchor)
    summary["num_workers"] = int(cfg.num_workers)
    summary["phase1_run_preshortlist_topm"] = int(getattr(cfg, "phase1_run_preshortlist_topm", 0))
    summary["phase1_full_run_score_topk"] = int(getattr(cfg, "phase1_full_run_score_topk", 0))
    summary["final_top_slice_reorder_enabled"] = bool(getattr(cfg, "final_top_slice_reorder_enabled", False))
    summary["final_top_slice_reorder_topk"] = int(getattr(cfg, "final_top_slice_reorder_topk", 0))
    summary["answer_support_pinning_enabled"] = bool(getattr(cfg, "answer_support_pinning_enabled", False))
    summary["answer_support_pinning_min"] = int(getattr(cfg, "answer_support_pinning_min", 0))
