    ...
    def retrieve(self,
        ...
        for q_idx, query in tqdm(enumerate(queries), desc="Retrieving", total=len(queries)):
            ...
            if len(top_k_facts) == 0 :
                ...
                query_graph = None  # DPR fast-path
            else:
                ...
                query_id = self.make_query_id(query) # `make_query_id` 메서드는 이미 존재합니다.
                query_graph = QueryGraphBuilder(
                    save_dir=f"./outputs/{self.global_config.dataset}/query_graph_analysis",   # 폴더는 취향대로
                    dataset_name=getattr(self.global_config, "dataset", "default")
                ).build(top_k_facts, query_id=query_id)                # reranked_doc_ids, reranked_scores = self.rerank_with_structure(query_graph, sorted_doc_ids[:num_to_retrieve], sorted_doc_scores[:num_to_retrieve])
                cfg = SubgraphConfig(max_depth=2, flow_quantile=0.75, subgraph_mode= "ppr",
                                     per_node_top_r=7, per_hop_topk =10, allow_topk=200, treat_undirected_for_structure=True)
                reranker = HyperReranker(
                    g=self.graph,
                    save_dir=f"./outputs/{self.global_config.dataset}/subgraph_analysis",
                    dataset_name=self.global_config.dataset   # ← 반드시 지정
                )                
                device = "cuda" if torch.cuda.is_available() else "cpu"
                embedder = HyperbolicEmbedder(hr_instance=self, embed_dim=32, curvature=1.0, device=device)
                # embedder = HyperbolicEmbedder(hr_instance=self,embed_dim=32, curvature=1.0, device="cpu")
                scorer   = StructralSimilarity(mode="cosine")

                # `use_semantic_features` 인자를 추가하여 의미 임베딩 결합 여부 제어
                # use_semantic_features = False  # <- (C) 패치에 따라 False로 설정
                from .utils.hyper_reranking_w_treerep import LTRConfig
                reranked_doc_ids, reranked_scores = reranker.rerank_with_structure(
  sim_cac_mode="ensemble",
  alpha=0.72, struct_k=22,
  gating_enabled=True, gate_floor=0.64, gate_ceiling=0.92,
  ensemble_ratio=0.60,
  radial_tau=0.5,
  landmark_k=10,
  treerep_backend="bfs", treerep_ppr_beta=None,  # ← BFS
  pin_k=2, k_rrf=24,
  lorentz_temp=2.0,
  use_semantic_features=True, use_treerep_metric=False,
  attn_pooling=False,
  ltr_config=None,
                    ...
                    # gating_enabled=True, # rho 기반 동적 게이팅은 활성화 (기본값)
                    # ensemble_ratio=0.6,
                    # radial_tau=0.5,
                    # landmark_k=12,
                    # treerep_backend = "treerep",
                    # treerep_ppr_beta=0.15,         # ★ 실험마다 바꿔보기
                    # pin_k=1,                       # ★ 0~1 권장
                    # k_rrf=20,                      # ★ 10/20 비교
                    # sim_cac_mode=getattr(self.global_config, "sim_cac_mode", "ensemble"),
                    # ltr_config = LTRConfig(
                    #     enabled=True,
                    #     weights=[0.8, 1.2, 0.7, 0.4, 0.6],  # [rrf_sem, rrf_struct, s, sem_norm, gate]
                    #     bias=0.0
                    # ),
                    # # --- 임베딩 관련 파라미터 ---
                    ...
                    # use_treerep_metric=False,  
                    
                    # --- 로깅 파라미터 ---
                    save_cb=self.saver,
                    save_ctx={"dataset_name": self.global_config.dataset, "query_id": query_id, "run_id": run_id, "mode": self.global_config.sim_cac_mode},
                    # lorentz_temp=getattr(self.global_config, "lorentz_temp", 2.0), # _struct_scores_lorentz에서 사용 가능
                )
                
                # use_semantic_features = True  # <- 이 값을 False로 변경하여 실험할 수 있습니다.
                # reranked_doc_ids, reranked_scores = reranker.rerank_with_structure(
                #     query_graph=query_graph,
                #     passage_topk_ids=sorted_doc_ids[:num_to_retrieve],
                #     passage_topk_scores=sorted_doc_scores[:num_to_retrieve],
                ...
                #     passage_embeddings=self.passage_embeddings,
                #     entity_embeddings=self.entity_embeddings,
                #     use_semantic_features=use_semantic_features,  # <- 추가된 인자
                #     use_treerep_metric=False,  
                #     # gating_enabled = False,          # ← 추가
                #     ensemble_ratio = 0.4,          # ← (선택) ensemble 가중
                #     node_ratio=0.00,           # ← NEW
                #     radial_tau = 0.5,
                #     landmark_k = 12,
                #     # alpha_min=0.756, alpha_max=0.88,
                #     # rho_low=-0.25, rho_high=0.35,
                #     k_rrf=20, pin_k=1,
                #     struct_norm = "robust",
                #     sem_struct_mode ="linear",
                #     struct_nonlin = "none",
                #     landmark_weighting = "none",
                #         # -------- 실험 토글 --------
                #     # enable_soft_pin = False,      # [실험2] 소프트 핀
                #     # soft_pin_margin_lo = 0.05,   # 1위-2위 상대 격차
                #     # soft_pin_margin_hi = 0.10,
                #     # topm_refine = True,          # [실험3] Top-M node-level 정밀 재정렬
                #     # topm_M = 20,
                #     # topm_refine_weight = 0.15,   # base 85% + refine 15%
                #     # topm_adaptive=True,  # ★
                #     stabilize_struct=False,      # 안정화 OFF

                #     save_cb=self.saver,
                #     save_ctx={"dataset_name": self.global_config.dataset, "query_id": query_id, "run_id": run_id, "mode": self.global_config.sim_cac_mode},
                #     sim_cac_mode=getattr(self.global_config, "sim_cac_mode", "tangent"),
                #     # lorentz_temp=getattr(self.global_config, "lorentz_temp", 2.0),

                # )
            


                top_k_docs = [self.chunk_embedding_store.get_row(self.passage_node_keys[idx])["content"] for idx in reranked_doc_ids]
                retrieval_results.append(QuerySolution(question=query, docs=top_k_docs, doc_scores=reranked_scores[:num_to_retrieve]))
