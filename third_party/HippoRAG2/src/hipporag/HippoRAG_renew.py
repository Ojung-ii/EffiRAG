...
class HippoRAG:
    ...
    def pre_openie(self,  docs: List[str]):
        ...
        raise RuntimeError("Offline OpenIE completed; switch to online mode for indexing/retrieval.")
    ...
    def retrieve(
        ...
        run_id = now_run_id()  # ✅ 여기서 1번만 만들기 (루프 밖)
        ...
        for q_idx, query in tqdm(enumerate(queries), desc="Retrieving", total=len(queries)):
            ...
            if len(top_k_facts) == 0:
                ...
                #     self.logger.warning(f"[QG] query_graph is None. Skip structure rerank. q_idx={q_idx}")
                #     reranked_doc_ids = sorted_doc_ids[:num_to_retrieve].tolist() if hasattr(sorted_doc_ids, "tolist") else list(sorted_doc_ids[:num_to_retrieve])
                #     reranked_scores  = sorted_doc_scores[:num_to_retrieve].tolist() if hasattr(sorted_doc_scores, "tolist") else list(sorted_doc_scores[:num_to_retrieve])
                #     dbg = None
                    
                if query_graph is None:
                    self.logger.warning(f"[QG] query_graph is None. Skip structure rerank. q_idx={q_idx}")

                    self._dump_json(case_dir / "04_hyper_before.json", {
                                "query": query, "q_idx": q_idx,
                                "hyper_applied": False,
                                "reason": "query_graph_none",
                                "fallback": "ppr_or_dense_only"
                            })
                    ...
                    retrieval_results.append(QuerySolution(question=query, docs=top_k_docs, doc_scores=reranked_scores))
                    continue  
                else:
                    qg_path = case_dir / "03_q_graph.graphml"
                    query_graph.write_graphml(str(qg_path))
                    self._dump_json(case_dir / "03_q_graph_meta.json", {
                        "nodes": int(query_graph.vcount()),
                        "edges": int(query_graph.ecount()),
                        "query_id": query_id,
                        "q_idx": q_idx,
                    })
                # case_dir = self._case_dir(query_id, run_id)
                ...
                k_rrf_cand = 120
                pids = np.array(sorted_doc_ids[:num_to_retrieve], dtype=int)
                sem_ranks = np.arange(1, len(pids) + 1)

                pid2node = {int(pid): int(self.passage_node_idxs[int(pid)]) for pid in pids}
                raw_ppr = self._extract_ppr_for_pids(
                    pids=pids,
                    last_ppr_scores=getattr(self, "last_ppr_scores", None),
                    passage_node_idxs=pid2node,   # ✅ dict로!
                    default_val=0.0,
                )
                ...
                before_payload = {
                    ...
                    "q_idx": q_idx,
                    "hyper_applied": True,
                    "k_rrf_cand": k_rrf_cand,
                    "alpha_rrf": 0.5,  # (설명용) sem/ppr을 rrf로 섞었다는 의미면 남기기
                    "candidates": [
                        {
                            "pid": int(pid),
                            "sem_rank": int(sr),
                        ...
                        for pid, sr, pr, r, pp, ss in zip(pids[order], sem_ranks[order], ppr_ranks[order], rrf[order], raw_ppr[order], np.asarray(sorted_doc_scores[:num_to_retrieve])[order])
                    ],
                    "hyper_input": [self._pid_entry(int(pid), float(sc)) for pid, sc in zip(pre_ids, pre_scores)],
                }
                self._dump_json(case_dir / "04_hyper_before.json", before_payload)
                ...
                reranked_doc_ids, reranked_scores, dbg = adaptive_rerank(
                    ...
                    reranker=reranker,  case_dir=case_dir,
...
# struct_k = 30            # 이전 로그에서 언급되었던 값 (추가적인 구조 필터링)
    ...
    #             )
            after_payload = {
                "query": query,
                "q_idx": q_idx,
                "hyper_output": [self._pid_entry(int(pid), float(sc)) for pid, sc in zip(reranked_doc_ids, reranked_scores)],
                "dbg": dbg 
                
                if dbg is None else {
                    # dbg가 무거우면 필요한 것만 추려서 저장 권장
            ...
            self._dump_json(case_dir / "05_hyper_after.json", after_payload)
            ...
            if dbg is not None:
                dbg_payload = {
                    ...
                    "reranked_doc_ids": dbg.get("reranked_doc_ids", reranked_doc_ids),
                    "reranked_scores": dbg.get("reranked_scores", reranked_scores),
                    "subgraph_paths": [str(p) for p in dbg.get("subgraph_paths", [])] if dbg.get("subgraph_paths") else None,
                    "subgraph_meta": dbg.get("subgraph_meta", None),
                }
    ...
    def graph_search_with_fact_entities(self, query: str,
        ...
        phrase_weights[valid] = phrase_weights[valid] / number_of_occurs[valid]
        phrase_weights[~valid] = 0.0
    ...
    def rerank_facts(self, query: str, query_fact_scores: np.ndarray) -> Tuple[List[int], List[Tuple], dict]:
        ...
        try:
            ...
            import ast
            candidate_facts = [ast.literal_eval(fact_row_dict[_id]["content"]) for _id in real_candidate_fact_ids]
    ...
    #             # ⚠️ 여기 절대 igraph.Graph 넣지마
    #             "subgraph_paths": dbg.get("subgraph_paths", None),
    #             "subgraph_meta": dbg.get("subgraph_meta", None),
    #         }
    ...
    def _dump_json(self, path: Path, obj):
        ...
        def _default(x):
            ...
            if torch.is_tensor(x):
                return x.detach().cpu().tolist()
            return str(x)  # 최후 fallback (그래도 graph 객체는 넣지 말 것)
    ...
    def _pid_entry(self, pid: int, score: float):
        key = self.passage_node_keys[pid]
        row = self.chunk_embedding_store.get_row(key)
        content = row["content"] if row and "content" in row else ""
        return {
            "pid": int(pid),
            "chunk_id": key,
            "score": float(score),
