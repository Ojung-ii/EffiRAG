...
class BaseConfig:
    ...
    llm_mode: Literal["online", "offline"] = field(
        default="online",
        metadata={"help": "Whether to use online(OpenAI) or offline(local) LLM for QA inference."}
    )
    ...
    # Final score and structural similarity
    alpha : float = field(
        default=0.7,
        metadata={"help": "Final score for Similarity Scores."}
    )

    struct_k: int = field(
        default=30,
        metadata={"help": "k for calculating structural similarity num."}
    )
