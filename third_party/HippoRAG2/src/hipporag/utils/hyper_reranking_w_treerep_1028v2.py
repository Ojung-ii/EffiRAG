...
# (igraph.Graph 는 해시 불가 → id(sg) 를 키로 사용)
# -------------------------------------------------------------------------------------
_TRE_CACHE: Dict[int, "TreeCache"] = {}

# ======================================================================================
# TreeCache (보강: 안전 LCA, 다컴포넌트 DFS, O(V) 직경 추정, radial, 가드)
# ======================================================================================
class TreeCache:
...
# ======================================================================================
# Build TreeCache from igraph (Treerep → BFS 폴백 + 캐시)
# ======================================================================================
def _build_treerep_cache_from_igraph(
    ...
    *,
    ...
    # treerep 사용 제한(노드/간선 캡) → BFS 폴백
    HARD_NODE_CAP = int(os.getenv("TREEREP_NODE_CAP", "120"))
    HARD_EDGE_CAP = int(os.getenv("TREEREP_EDGE_CAP", "400"))
    force_bfs = (backend != "treerep") or (gu.vcount() > HARD_NODE_CAP) or (gu.ecount() > HARD_EDGE_CAP)

    if not force_bfs and gu.ecount() > 0:
        try:
            eps = 1e-6
            if has_w:
            ...
            # (중요) 이전: all-pairs shortest_paths → 병목
            # Treerep 학습은 유지하되, 그래프 크기가 작을 때만 허용
            ...
            treerep = TreeRep(d=D); treerep.learn_tree(); Tloc = treerep.G
...
# ======================================================================================
# MetricsLogger & compute_struct_metrics (그대로)
# ======================================================================================
class MetricsLogger:
...
# ======================================================================================
# HyperbolicEmbedder (treerep all-pairs 제거: 내부 트리 필요시 _build_treerep_cache_from_igraph 사용)
# ======================================================================================
class HyperbolicEmbedder:
    ...
    # (수정) 트리 필요 시 TreeCache 이용. 거리행렬 D는 사용하지 않으므로 0행렬 반환.
    def _get_treerep_tree(self, g: ig.Graph, ppr_scores: Optional[np.ndarray]=None, treerep_ppr_beta: Optional[float]=None):
        n=g.vcount()
    ...
    def graph_to_tensor(self, g: ig.Graph, ppr_scores: Optional[np.ndarray]=None,
        ...
        if use_treerep_metric:
            _, T = self._get_treerep_tree(g, ppr_scores=ppr_scores, treerep_ppr_beta=treerep_ppr_beta)
    ...
    # (편의) 탄젠트 정규화/공유 표준화/어텐션 풀링(노드레벨) 유틸
...
class HyperReranker:
    ...
    def _struct_scores_gromov(self, query_graph: ig.Graph, subgraphs: List[ig.Graph], landmark_k:int=10,
        ...
        def _build_cache(sg):
            return _build_treerep_cache_from_igraph(sg, backend=treerep_backend,
                                                    ppr_scores=ppr_scores,
                                                    treerep_ppr_beta=treerep_ppr_beta)
        Qud=query_graph.as_undirected() if query_graph.is_directed() else query_graph
        qcache=_build_cache(Qud)
        if qcache is None: return np.zeros(len(subgraphs))
        q_lm=self._pick_landmarks_diverse(qcache, landmark_k)

        def _distmat(cache: TreeCache, nodes):
            L=len(nodes); M=np.zeros((L,L),dtype=np.float32)
            for i in range(L):
                for j in range(i+1,L):
                    d=cache.dist(nodes[i], nodes[j])  # ★ TreeCache 거리 사용(경량)
                    M[i,j]=M[j,i]=d
        ...
        sims=[]
        for sg in subgraphs:
            dcache=_build_cache(sg)
            if dcache is None: sims.append(0.0); continue
            d_lm=self._pick_landmarks_diverse(dcache, min(landmark_k, len(dcache.T.nodes())))
            L=min(len(q_lm), len(d_lm))
            if L<=1: sims.append(0.0); continue
            QL=Q[:L,:L]; DL=_distmat(dcache, d_lm)[:L,:L]
    ...
    def rerank_with_structure(self, query_graph: Optional[ig.Graph],
        ...
        # 구조 점수
        if sim_cac_mode=="node_level" or (sim_cac_mode=="tangent" and attn_pooling):
            s = self._struct_scores_tangent(
                ...
                attn_pooling=True, attn_lambda=attn_lambda, attn_topk=attn_topk, attn_residual=attn_residual
            )
        elif sim_cac_mode=="tangent":
            s = self._struct_scores_tangent(
                query_graph, subgraphs, embedder, scorer, ppr_scores=last_ppr_scores,
                passage_embeddings=passage_embeddings, entity_embeddings=entity_embeddings,
                use_semantic_features=use_semantic_features, use_treerep_metric=use_treerep_metric,
            ...
            s = self._struct_scores_struct_feat(
            ...
            s = ensemble_ratio*s1 + (1.0-ensemble_ratio)*s2
        else:
