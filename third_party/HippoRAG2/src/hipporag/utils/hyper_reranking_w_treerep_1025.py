...
# import os
# import pandas as pd
# # from scipy.stats import spearmanr, kendalltau # moved to inside rerank_with_structure for guarded import
...
# # scipy 없는 환경 대비: Spearmanr 가드 추가
# try:
#     from scipy.stats import spearmanr, kendalltau
# except ImportError:
#     class R: correlation = 0.0
#     def spearmanr(a, b): return R()
#     def kendalltau(a, b): return R()
...
#         root_gid = int(gu["root_id"])
#         if root_gid not in gids:  # 안전가드
#             root_gid = gids[0]
#     else:
#         root_gid = gids[0]
...
#                 # (B) TreeRep penalty 튜닝 노출 적용: beta 값을 환경 변수에서 로드
...
#             'leaf': self._get_leaf, 
#             # 'eig_centrality': _get_eig_centrality, 
#             # 'clustering': _get_clustering, 
#             # 'pr_undirected': _get_pr_undirected, 
#             # 'katz': _get_katz, 
#         }
...
#         n = g.vcount()
#         if n < 2: return np.zeros((n, n), dtype=np.float32), nx.Graph()
        
#         gu: igraph.Graph = g.as_undirected() if g.is_directed() else g
#         eps = 1e-6
#         # (A) 간선 가중치 → 거리 변환: 1/(eps + w)
...
#             pv = np.nan_to_num(np.asarray(ppr_scores, dtype=float), nan=0.0)
#             pv = np.clip(pv, 0.0, None)
#             with np.errstate(divide='ignore'): pen_nodes = -np.log(pv + eps)
            
#             # # beta 값은 이미 _build_treerep_cache_from_igraph에서 환경 변수 로직이 적용됨. 여기는 0.25 기본값 사용
#             # beta = 0.25 
...
#         # (C) 최단거리 행렬
#         try: distances = np.array(gu.shortest_paths(weights=w_dist), dtype=np.float32)
#         except Exception: distances = np.array(gu.shortest_paths(), dtype=np.float32) # fallback
            
#         if np.isinf(distances).any(): # 무한대 보정
...
#                 # NOTE: self.hr.global_config.embedding_dim 가정
#                 embed_dim = self.embed_dim
#                 if hasattr(self.hr, 'global_config') and hasattr(self.hr.global_config, 'embedding_dim'):
#                     embed_dim = self.hr.global_config.embedding_dim
#                 embed = np.zeros(embed_dim, dtype=np.float32)
...
#                 # 임베딩 차원 문제 방지를 위해, 일단 features가 있다면 hstack에 넣음.
...
#                     T.norm(dim=1).mean().item(), T.std().item(), vals.mean().item(), 
...
#                 sim = 1.0 / (1.0 + np.exp(-((norm_sim - 0.5) / (1.0 / temp)))) # 로지스틱 변환을 통해 유사도 점수 산출
...
#         self, query_graph: ig.Graph, subgraphs: List[ig.Graph], landmark_k: int = 10, 
#         ppr_scores: Optional[np.ndarray] = None,
#     ) -> np.ndarray:
...
#             backend="treerep", ppr_scores=ppr_scores,
#         )
...
#             dcache = _build_treerep_cache_from_igraph( sg, backend="treerep", ppr_scores=ppr_scores )
...
#             score = 1.0 - float(np.mean(np.abs(QL - DL))) # 거리 차이의 평균을 유사도로 변환
...
#         self, query_graph: igraph.Graph, subgraphs: List[ig.Graph], query_radial_anchor: Optional[float] = None, 
#         radial_tau: float = 0.5, ppr_scores: Optional[np.ndarray] = None,
#     ) -> np.ndarray:
#         # 쿼리 rq 추정(중앙값 반지름)
#         rq = 0.5
...
#                     qnodes = [
#                         int(x) for x in (
#                             query_graph.vs["global_id"] if "global_id" in query_graph.vs.attributes() else list(range(query_graph.vcount()))
#                         )
#                     ]
...
#             cand = [
#                 int(x) for x in (
#                     sg.vs["global_id"] if "global_id" in sg.vs.attributes() else list(range(sg.vcount()))
#                 )
#             ]
...
#             # 특징 가중치 (QA-지향)
#             w = {
#                 "radial_gap_mean": 0.35, "geo_coherence": 0.30, "steiner_coverage": 0.25, 
#                 "gp_mean": 0.05, "branch_focus": 0.05,
#             }
#             s = 0.0
#             for k, a in w.items(): s += a * float(feats.get(k, 0.0))
...
#         self, query_graph: Optional[ig.Graph], passage_topk_ids: List[int], passage_topk_scores: List[float], 
#         passage_node_idxs: List[int], last_ppr_scores: Optional[np.ndarray], cfg: SubgraphConfig, 
#         embedder: HyperbolicEmbedder, scorer: StructralSimilarity, passage_embeddings: np.ndarray, 
#         entity_embeddings: np.ndarray, use_semantic_features: bool = True, use_treerep_metric: bool = False, 
#         gating_enabled: bool = True, ensemble_ratio: float = 0.6, radial_tau: float = 0.5, 
#         landmark_k: int = 10, alpha: float = 0.8, struct_k: int = 30, device: str = "cpu", 
#         save_cb: Optional[Callable[[ig.Graph, str, dict], None]] = None, save_ctx: Optional[dict] = None, 
...
#         # 2. 구조적 유사도 계산 (모드 선택)
#         _use_semantic = use_semantic_features
#         if sim_cac_mode in ["tangent", "lorentz", "node_level"]:
#             # (C) 임베더 구조모드 의미피처 off 적용
#             _use_semantic = False
...
#                 use_semantic_features=_use_semantic, use_treerep_metric=use_treerep_metric
#             )
...
#             s1 = self._struct_scores_struct_feat(query_graph, subgraphs, None, radial_tau, ppr_scores=last_ppr_scores)
#             s2 = self._struct_scores_gromov(query_graph, subgraphs, landmark_k=landmark_k, ppr_scores=last_ppr_scores)
...
#         # in HyperReranker.rerank_with_structure(...)
#         # replace the whole fusion section (RRF/gate/pin…) with the C2 combiner:
...
#         # ③ C2 결합
#         final_head = alpha * sem_norm_head + (1.0 - alpha) * s
...
#         try :
#             logger.info(
#                 "[EXP_C2-DIAG] head_k=%d alpha=%.2f | "
#                 "sem[min=%.4f max=%.4f] struct[min=%.4f max=%.4f] | "
#                 "pair_top5=%s",
#                 struct_k, alpha,
#                 float(sem_norm_head.min()), float(sem_norm_head.max()),
#                 float(s.min()) if s.size else 0.0, float(s.max()) if s.size else 0.0,
#                 str([hp[0] for hp in head_pairs[:5]])
#             )
#         except Exception:
#             pass
...
#         # # 2) RRF(Reciprocal Rank Fusion)로 스케일 이슈 제거
