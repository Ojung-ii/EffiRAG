...
class QuerySolution:
    ...
    def to_dict(self):
        scores = self._to_pylist(self.doc_scores)  # list/ndarray/tensor -> list
        scores_out = None
        if scores is not None:
            # numpy.float32 등 대비해서 float 캐스팅
            scores_out = [round(float(v), 4) for v in scores[:5]]
        ...
        return {
            ...
            "docs": self.docs[:5],
            "doc_scores": scores_out,
            # "doc_scores": [round(v, 4) for v in self.doc_scores[:5]]  if self.doc_scores is not None else None,
