...
class StructFeatureExtractor:
    ...
    # [UPDATE] 가중 평균 지원 (radial/gp_mean)
    def features_for_group(self, nodes: List[int], rq: float, tau: float=0.5,
                           anchor: Optional[int]=None,
                           weights: Optional[Mapping[int,float]] = None) -> Dict[str,float]:
        if not nodes: 
        ...
        ws = None
        if weights:
            ws = np.asarray([max(0.0,float(weights.get(v,0.0))) for v in nodes], dtype=np.float32)
            if ws.sum() <= 1e-9: ws = None
            else: ws = ws / (ws.sum()+1e-9)

        rads=[self.radial_gap(v,rq,tau) for v in nodes]
        radial_gap_mean=float(np.average(rads, weights=ws) if ws is not None else np.mean(rads))

        geo=self.geodesic_coherence(nodes)
        cov=self.steiner_coverage(nodes)
        gp=self.gromov_anchor_summary(anchor, nodes)

        gp_mean = float(np.average(
            [self.gromov_anchor_summary(anchor,[v])["gp_mean"] for v in nodes],
            weights=ws) if ws is not None else gp["gp_mean"]
        )
...
def compute_struct_metrics(
    ...
    *, 
    ...
    try:
        ...
        m["path_len_mean"] = 0.0; m["path_len_p95"] = 0.0
    ...
    try:
        ...
        m["betw_p90"] = 0.0; m["betw_frac_gt_p90"] = 0.0; m["betw_gini"] = 0.0
...
class HyperbolicEmbedder:
    def __init__(self, hr_instance, embed_dim=32, curvature=1.0, device="cpu",
                 pool_mode: Literal["deg","ppr"]="deg"):  # [UPDATE] 풀링 모드
    ...
    def set_curvature(self, k: float):  # [UPDATE]
        self.manifold = geoopt.manifolds.Lorentz(k=k)
    ...
    def _get_treerep_tree(self, g: ig.Graph, ppr_scores: Optional[np.ndarray] = None, treerep_ppr_beta: Optional[float] = None) -> Tuple[np.ndarray, nx.Graph]:
        ...
        if ppr_scores is not None and "global_id" in gu.vs.attributes():
            ...
            beta = treerep_ppr_beta if treerep_ppr_beta is not None else float(os.getenv("TREEREP_PPR_BETA", "0.25"))
            add_cost = []
            for e in gu.es:
                u, v = e.source, e.target
    ...
    def _treerep_features_from_tree(self, learned_tree: nx.Graph, g: ig.Graph) -> np.ndarray:
        ...
        feats = np.stack([
            depth_norm, leaf, parent_w_norm, is_root, subtree_size_norm, branching_factor_norm, centroid_flag
    ...
    def graph_to_tensor(
        ...
        for _, extractor_func in self.feature_extractors.items():
        ...
        X = np.hstack(feature_list) if feature_list else np.zeros((g.vcount(), self.embed_dim), dtype=np.float32)
    ...
    def _node_weights(self, g: ig.Graph, ppr_scores: Optional[np.ndarray]) -> torch.Tensor:
        """[UPDATE] deg/ppr 가중 (Euclidean 공간에서 가중 평균 뒤 expmap0)"""
        n = g.vcount()
        if n == 0: return torch.ones(0, device=self.device)
        if self.pool_mode == "ppr" and ppr_scores is not None and "global_id" in g.vs.attributes():
            gids = np.asarray(g.vs["global_id"], dtype=int)
            w = np.nan_to_num(np.asarray(ppr_scores, dtype=float)[gids], nan=0.0)
        else:
            w = np.asarray(g.degree(mode="all"), dtype=float)
        w = np.clip(w, 0.0, None)
        s = float(w.sum())
        if s <= 1e-9: w = np.ones(n, dtype=float)/max(1,n)
        else: w = w / s
    ...
    def embed_lorentz(
        ...
        with torch.no_grad():
            if g.vcount() > 1:
                max_norm = 5.0
                nrm = torch.linalg.norm(x, dim=1, keepdim=True) + 1e-6
                x = x * torch.clamp(max_norm / nrm, max=1.0)
                w = self._node_weights(g, ppr_scores).unsqueeze(1)  # [N,1]
                x_bar = (w * x).sum(dim=0)  # [D]  # [UPDATE] 가중 평균 후
                pooled = self.manifold.expmap0(x_bar.unsqueeze(0)).squeeze(0)  # 로런츠로 이동
            else:
                pooled = self.manifold.expmap0(x).squeeze(0)
...
class HyperReranker:
    ...
    def _struct_scores_lorentz(
        ...
        with torch.no_grad():
            ...
            dL_tensors = [x if x is not None else torch.zeros(embedder.embed_dim, device=embedder.device) for x in dL]
            dists2 = torch.stack([embedder.manifold.dist2(qL.unsqueeze(0), x.unsqueeze(0)).squeeze() for x in dL_tensors])
    ...
    def _struct_scores_struct_feat(
        ...
        for sg in subgraphs:
            ...
            # [UPDATE] PPR/deg 가중 벡터 구성
            cand = [int(x) for x in (sg.vs["global_id"] if "global_id" in sg.vs.attributes() else range(sg.vcount()))]
            weights = None
            if ppr_scores is not None and len(cand) > 0:
                pv = np.nan_to_num(np.asarray(ppr_scores, dtype=float), nan=0.0)
                w = {gid: float(pv[gid]) for gid in cand}
                if sum(w.values()) > 0: weights = w
            if weights is None:
                degs = dict((gid, float(dcache.T.degree(gid))) for gid in cand)
                if sum(degs.values()) > 0: weights = degs
            ext = StructFeatureExtractor(dcache)
            feats = ext.features_for_group(cand, rq, tau=radial_tau, anchor=None, weights=weights)  # [UPDATE]
            wmix = {"radial_gap_mean": 0.35, "geo_coherence": 0.30, "steiner_coverage": 0.25, "gp_mean": 0.05, "branch_focus": 0.05}
            s = sum(a * float(feats.get(k, 0.0)) for k, a in wmix.items())
            scores.append(max(0.0, min(1.0, s)))
        return np.asarray(scores, dtype=float)

    # [UPDATE] β 적응(간단 규칙)
    def _adaptive_treerep_beta(self, ppr_scores: Optional[np.ndarray], graphs: List[ig.Graph]) -> float:
        if ppr_scores is None or not graphs: return float(os.getenv("TREEREP_PPR_BETA", "0.25"))
        gids = []
        for sg in graphs:
            if "global_id" in sg.vs.attributes():
                gids.extend(list(map(int, sg.vs["global_id"])))
        if not gids: return 0.25
        pv = np.nan_to_num(np.asarray(ppr_scores, dtype=float), nan=0.0)
        sel = pv[gids]
        s = sel.sum()
        if s <= 0: return 0.25
        p = sel / s
        H = _entropy(p)
        # Heuristic: 희소/허브지배(H 낮음) -> β↑, 균등(H 높음) -> β↓
        return 0.30 if H < 2.0 else (0.20 if H < 3.0 else 0.15)
    ...
    def rerank_with_structure(
        ...
        k_rrf: int = 20,
        # [UPDATE] 하이퍼 옵션들
        kappa_candidates: Tuple[float,...] = (0.5, 1.0, 2.0),
        hyp_ratio: float = 0.15,              # 3-way 앙상블에서 하이퍼비중 η
        pin_floor_epsilon: float = 1e-3,      # semantic Top-1 바닥치
        ...
        # 미리 sem_head 산출 (κ 선택 등에 사용)
        sem = np.asarray(passage_topk_scores, float)
        head_idx = np.argsort(-sem)[:len(subgraphs)]
        sem_head = sem[head_idx]

        # [UPDATE] β 적응 (필요 시)
        if treerep_ppr_beta is None:
            treerep_ppr_beta = self._adaptive_treerep_beta(last_ppr_scores, subgraphs)

        # 구조점수 계산
        def _scores_for_mode(mode: str, _embedder: HyperbolicEmbedder) -> np.ndarray:
            if mode == "tangent":
                return self._struct_scores_tangent(
                    query_graph, subgraphs, _embedder, scorer, ppr_scores=last_ppr_scores, 
            ...
            if mode == "lorentz":
                return self._struct_scores_lorentz(
                    query_graph, subgraphs, _embedder, temp=lorentz_temp, ppr_scores=last_ppr_scores, 
                    passage_embeddings=passage_embeddings, entity_embeddings=entity_embeddings, 
                    use_semantic_features=use_semantic_features, use_treerep_metric=use_treerep_metric
                )
            if mode == "node_level":
                return self._struct_scores_node_level(
                    query_graph, subgraphs, _embedder, ppr_scores=last_ppr_scores, 
                    passage_embeddings=passage_embeddings, entity_embeddings=entity_embeddings, 
                    use_semantic_features=use_semantic_features, use_treerep_metric=use_treerep_metric
                )
            if mode == "gromov":
                return self._struct_scores_gromov(
            ...
            if mode == "struct_feat":
                return self._struct_scores_struct_feat(
                    query_graph, subgraphs, query_radial_anchor=None, radial_tau=radial_tau, ppr_scores=last_ppr_scores,
                    treerep_backend=treerep_backend, treerep_ppr_beta=treerep_ppr_beta
                )
            raise ValueError(f"Unknown mode: {mode}")

        # [UPDATE] κ 초경량 탐색(필요할 때만: tangent 점수 산출용)
        tangent_scores = None
        if sim_cac_mode in ("tangent","ensemble"):
            best_rho, best_k, best_scores = -1.0, None, None
            for kappa in kappa_candidates:
                emb_local = HyperbolicEmbedder(embedder.hr, embed_dim=embedder.embed_dim, curvature=kappa,
                                               device=embedder.device, pool_mode=embedder.pool_mode)
                s_tmp = _scores_for_mode("tangent", emb_local)
                if s_tmp.size == 0: continue
                s_norm = StructralSimilarity().robust_normalize_struct(s_tmp)
                try:
                    rho = spearmanr(sem_head, s_norm).correlation
                    rho = 0.0 if rho is None or np.isnan(rho) else abs(float(rho))
                except Exception:
                    rho = 0.0
                if rho > best_rho:
                    best_rho, best_k, best_scores = rho, kappa, s_norm
            if best_scores is None:
                tangent_scores = np.zeros(len(subgraphs))
            else:
                embedder.set_curvature(best_k)
                tangent_scores = best_scores

        if sim_cac_mode == "tangent":
            struct_head = tangent_scores
        elif sim_cac_mode in ("lorentz","node_level","gromov","struct_feat"):
            struct_head = _scores_for_mode(sim_cac_mode, embedder)
        elif sim_cac_mode == "ensemble":
            s1 = _scores_for_mode("struct_feat", embedder)
            s2 = _scores_for_mode("gromov", embedder)
            # [UPDATE] 3-way: + η * tangent
            s3 = tangent_scores if tangent_scores is not None else np.zeros_like(s1)
            base = ensemble_ratio * s1 + (1.0 - ensemble_ratio) * s2
            struct_head = (1.0 - hyp_ratio) * base + hyp_ratio * s3
        else:
        ...
        # 강건 정규화 + RRF
        s = StructralSimilarity().robust_normalize_struct(np.asarray(struct_head, float))
        ...
        gate = (np.clip(0.6 + 0.4 * max(0.0, rho), 0.6, 1.0) if gating_enabled else 1.0)
        if gating_enabled and rho <= -0.2:
            rrf_struct = 1.0 - rrf_struct

        final_head = alpha * rrf_sem + (1.0 - alpha) * gate * rrf_struct

        # [UPDATE] ε 바닥치(semantic top-1 낙하 방지, 고정핀 대체)
        if pin_floor_epsilon > 0 and len(sem_head) > 0:
            top1_local = int(np.argmax(sem_head))
            final_head[top1_local] += float(pin_floor_epsilon)
        ...
        try:
            logger.info(f"[FUSION] k_rrf={k_rrf} gate={gate:.3f} rho={rho:.3f} beta={treerep_ppr_beta} pinned={pinned_ids}")
        except Exception:
            pass
